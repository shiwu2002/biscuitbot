...
本文档描述 xianaibot 的 LLM Provider 子系统与聊天平台 Channel 子系统的设计与实现。

## 1. Provider 系统概览

Provider 子系统负责将统一的 chat/chat_stream 调用适配到不同 LLM 厂商。核心代码位于 [providers/](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/) 目录。

- **注册表**：[registry.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/registry.py) 中的 `PROVIDERS` 元组当前登记 9 个提供商（custom、anthropic、openai、deepseek、dashscope、zhipu、moonshot、stepfun、ollama），元组顺序即匹配优先级，网关类优先。
- **后端实现**：2 种 backend —— `openai_compat`（OpenAI 兼容协议，覆盖绝大多数厂商）与 `anthropic`（原生 Anthropic SDK）。backend 字段决定工厂分发到哪个实现类。
- **三大模块**：基类 [base.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/base.py)、工厂 [factory.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/factory.py)、注册表 [registry.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/registry.py)，外加故障转移包装器 [fallback_provider.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/fallback_provider.py)。

## 2. LLMProvider 基类

[base.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/base.py) 中的 `LLMProvider(ABC)` 定义所有 provider 的统一契约与重试策略。
...
通过环境变量 `XIANAIBOT_STREAM_IDLE_TIMEOUT_S` 配置，`resolve_stream_idle_timeout_s` 负责安全解析：默认 `90.0` 秒，上限 `3600.0` 秒；非法或非正值回退默认，超过上限则钳制。超时后视为瞬时错误触发重试。
...
[factory.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/factory.py) 负责从配置创建 provider 实例。
...
[registry.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/registry.py) 是 provider 元数据的唯一真相源。
...
[fallback_provider.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/fallback_provider.py) 的 `FallbackProvider(LLMProvider)` 包装主 provider 并透明切换到备用模型。
...
- **图像生成**：[image_generation.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/image_generation.py) 定义 `ImageGenerationError` 与 `GeneratedImageResponse`（images、content、raw），支持 DashScope 万相、Zhipu CogView、Gemini Imagen、Ollama 等多端点，默认超时 120 秒。
- **语音转写**：[transcription.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/providers/transcription.py) 适配 OpenAI Whisper、Groq 等兼容端点。`_resolve_transcription_url` 兼容 chat 风格 base 与完整 `/audio/transcriptions` 路径。自带重试：`_MAX_RETRIES = 3`、`_BACKOFF_S = (1.0, 2.0, 4.0)`、`_RETRYABLE_STATUS = {408, 429, 500, 502, 503, 504}`，覆盖 Whisper 端点偶发 502/503 与移动网络读写错误。

## 7. Channel 系统概览

Channel 子系统适配各聊天平台，核心位于 [channels/](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/) 目录。当前内置 **9 个** channel 模块，通过 [registry.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/registry.py) 的自动发现机制加载，并支持经 `entry_points` 注册的外部插件。

## 8. BaseChannel 抽象基类

[base.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/base.py) 的 `BaseChannel(ABC)` 定义三个抽象方法：
...
[manager.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/manager.py) 的 `ChannelManager` 统管 channel 生命周期与消息路由。
...
[registry.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/registry.py) 提供三层发现：

- `discover_channel_names()`：用 `pkgutil.iter_modules` 扫描 `xianaibot.channels` 包，排除 `base`/`manager`/`registry` 内部模块，**不触发第三方 SDK 导入**。
- `load_channel_class(module_name)`：导入模块并返回首个 `BaseChannel` 子类。
- `discover_plugins(enabled_names)`：经 `importlib.metadata.entry_points(group="xianaibot.channels")` 加载外部插件；内置 channel 优先，插件不得遮蔽同名内置项。
...
| 名称 | 文件 | 说明 |
|------|------|------|
| dingtalk | [dingtalk.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/dingtalk.py) | 钉钉机器人 |
| email | [email.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/email.py) | 电子邮件（IMAP/SMTP） |
| feishu | [feishu.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/feishu.py) | 飞书 |
| mochat | [mochat.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/mochat.py) | MoChat |
| napcat | [napcat.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/napcat.py) | NapCat（QQ 协议） |
| qq | [qq.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/qq.py) | QQ |
| websocket | [websocket.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/websocket.py) | WebSocket（WebUI 网关） |
| wecom | [wecom.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/wecom.py) | 企业微信 |
| weixin | [weixin.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/channels/weixin.py) | 微信 |

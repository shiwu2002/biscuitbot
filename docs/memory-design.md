...
xianaibot 的记忆系统是一个**两层 + 两阶段**的混合记忆架构，旨在让 Agent 跨会话保留长期信息，同时控制上下文窗口的 token 成本。
...
核心代码位置：[xianaibot/agent/memory.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py)
...
记忆系统管理四类文件，由 [MemoryStore](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L40) 统一管理，遵循 **MECE（相互独立、完全穷尽）** 原则：
...
Dream 模板（[templates/agent/dream.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/agent/dream.md)）定义了严格的路由规则：
...
- `history.jsonl` 和 `.dream_cursor` 受 Shell 工具保护，禁止直接写入（[tools/shell.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/tools/shell.py#L189)）
- 模板内容检测：[ContextBuilder._is_template_content](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/context.py#L179) 会跳过未自定义的模板文件，避免注入占位符内容

---

## 4. MemoryStore：纯文件 I/O 层

[MemoryStore](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L40) 是记忆系统的底层存储抽象，只负责文件读写，不涉及 LLM 调用。
...
- **cursor 分配 + 追加**：使用 `threading.Lock` 串行化（[memory.py:275](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L275)），防止并发写入产生重复 cursor
- **compact_history 加锁**：`compact_history` 在 `_append_lock` 内执行读-改-写（[memory.py:387](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L387)），防止并发 `append_history` 的条目被原子重写覆盖丢失
- **整文件重写**：`_write_entries` 使用 temp 文件 + `os.replace` + `fsync` + 目录 `fsync` 保证原子性和持久性（[memory.py:483](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L483)）
- **游标文件原子写入**：`.cursor` 和 `.dream_cursor` 均使用 `atomic_write_text`（temp + rename + fsync）写入（[helpers.py:355](file:///Volumes/data/hczkAgent/nanobot/xianaibot/utils/helpers.py#L355)），崩溃时不会产生部分写入
...
`compact_history()` 在条目数超限时丢弃最旧的条目，**但不会静默删除**——被驱逐的条目归档到 `history-archive-YYYY-MM.jsonl`（[memory.py:406](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L406)），保留审计可追溯性。

### 4.6 `_read_last_entry` 健壮读取

`_read_last_entry`（[memory.py:437](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L437)）用于高效读取 history.jsonl 的最后一条记录（避免全文件扫描来分配下一个 cursor）：
...
[Consolidator](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L623) 负责在会话进行中，当 prompt token 接近上下文窗口时，将旧消息摘要归档到 `history.jsonl`。
...
`maybe_consolidate_by_tokens` 的核心循环（[memory.py:862](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L862)）：
...
`archive` 方法（[memory.py:855](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L855)）：

1. 格式化消息为 `[timestamp] ROLE [tools: ...]: content` 形式
2. 截断到 token 预算内
3. 调用 LLM，使用 [consolidator_archive.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/agent/consolidator_archive.md) 模板，注入 `session_key` 上下文（多会话场景下帮助 LLM 区分不同渠道/聊天的事实）
...
Dream 是 xianaibot 记忆系统的核心创新，采用**两阶段**设计将短期会话历史沉淀为长期结构化记忆。
...
- **常量复用**：`cmd_dream` 和 cron job 共享 `_DREAM_MAX_BATCHES` 常量（[builtin.py:20](file:///Volumes/data/hczkAgent/nanobot/xianaibot/command/builtin.py#L20)）

#### 6.3.2 提示词构建

`build_dream_prompt`（[memory.py:467](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L467)）：

1. 读取 `.dream_cursor`，获取未处理的历史条目
2. 取最近 `max_entries=20` 条
3. 每条截断到 500 字符
4. 渲染 [dream.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/agent/dream.md) 模板
5. 拼接历史上下文

#### 6.3.3 受限工具集

`build_dream_tools`（[memory.py:491](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L491)）构建一个**受限**的 ToolRegistry，仅包含：
...
[dream.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/agent/dream.md) 模板的核心设计：
...
[AutoCompact](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/autocompact.py) 处理长时间空闲的会话，避免累积过多历史。
...
`compact_idle_session`（[memory.py:971](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L971)）：
...
[ContextBuilder](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/context.py#L51) 负责将记忆注入到 LLM 的系统提示词中。
...
`read_recent_history_for_prompt`（[memory.py:365](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py#L365)）：
...
当未处理历史超过 `_MAX_RECENT_HISTORY`（50 条）时，`_format_recent_history`（[context.py:184](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/context.py#L184)）不会静默丢弃早期条目，而是将积压区**最接近 recent 窗口的 20 条**（`backlog[-20:]`）截断为 100 字符摘要，以 `[Backlog]` 前缀注入：
...
[GitStore](file:///Volumes/data/hczkAgent/nanobot/xianaibot/utils/gitstore.py#L45) 使用 dulwich（纯 Python Git 实现）为记忆文件提供版本控制。
...
- 提交消息包含 Dream 摘要内容
- 作者固定为 `xianaibot <xianaibot@dream>`
...
定义于 [config/schema.py:52](file:///Volumes/data/hczkAgent/nanobot/xianaibot/config/schema.py#L52)：
...
Agent 可通过内置 `grep` 工具搜索 `history.jsonl`（见 [skills/memory/SKILL.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/skills/memory/SKILL.md)）：
...
- `atomic_write_text`（[helpers.py:355](file:///Volumes/data/hczkAgent/nanobot/xianaibot/utils/helpers.py#L355)）统一处理 `.cursor`、`.dream_cursor`
...
| 文件 | 职责 |
|------|------|
| [xianaibot/agent/memory.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/memory.py) | MemoryStore + Consolidator 核心 |
| [xianaibot/agent/context.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/context.py) | 上下文构建与记忆注入（含 Backlog 摘要） |
| [xianaibot/agent/autocompact.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/autocompact.py) | 空闲会话自动压缩 |
| [xianaibot/agent/loop.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/agent/loop.py) | Agent 主循环，整合触发点 |
| [xianaibot/utils/helpers.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/utils/helpers.py) | `atomic_write_text` 原子写入工具 |
| [xianaibot/utils/gitstore.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/utils/gitstore.py) | Git 版本控制 |
| [xianaibot/templates/agent/dream.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/agent/dream.md) | Dream 提示词模板 |
| [xianaibot/templates/agent/consolidator_archive.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/agent/consolidator_archive.md) | Consolidator 摘要模板（含 session_key 上下文） |
| [xianaibot/templates/memory/MEMORY.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/memory/MEMORY.md) | MEMORY.md 模板 |
| [xianaibot/templates/SOUL.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/SOUL.md) | SOUL.md 模板 |
| [xianaibot/templates/USER.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/templates/USER.md) | USER.md 模板 |
| [xianaibot/skills/memory/SKILL.md](file:///Volumes/data/hczkAgent/nanobot/xianaibot/skills/memory/SKILL.md) | 记忆系统技能说明 |
| [xianaibot/command/builtin.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/command/builtin.py) | /dream 系列命令 + `_DREAM_MAX_BATCHES` 常量 |
| [xianaibot/config/schema.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/config/schema.py) | DreamConfig 配置 |
| [xianaibot/cli/commands.py](file:///Volumes/data/hczkAgent/nanobot/xianaibot/cli/commands.py) | Dream cron 任务（多批次循环） |

"""Tests for the Xianaibot programmatic facade."""
...
from xianaibot.xianaibot import Xianaibot, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Xianaibot.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Xianaibot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("xianaibot.config.loader.load_config") as mock_load, \
         patch("xianaibot.providers.factory.make_provider") as mock_prov:
        mock_load.return_value = Config()
        mock_prov.return_value = MagicMock()
        mock_prov.return_value.get_default_model.return_value = "test"
        mock_prov.return_value.generation.max_tokens = 4096
        Xianaibot.from_config()
...
async def test_run_with_hooks(tmp_path):
    from xianaibot.agent.hook import AgentHook, AgentHookContext
    from xianaibot.bus.events import OutboundMessage
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from xianaibot.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = Xianaibot.from_config(config_path, workspace=custom_ws)
...
def test_import_from_top_level():
    ...
    assert xianaibot.Xianaibot is Xianaibot
    assert xianaibot.RunResult is RunResult
...
async def test_run_populates_tools_used_across_iterations(tmp_path):
    ...
    from xianaibot.agent.hook import AgentHookContext
    from xianaibot.bus.events import OutboundMessage
    from xianaibot.providers.base import ToolCallRequest
...
async def test_sdk_capture_prefers_run_level_snapshot():
    from xianaibot.agent.hook import AgentHookContext, AgentRunHookContext, SDKCaptureHook
    from xianaibot.providers.base import ToolCallRequest

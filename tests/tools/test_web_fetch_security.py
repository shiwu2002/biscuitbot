...
from hczkbot.agent.tools import web as web_module
from hczkbot.agent.tools.web import WebFetchTool
from hczkbot.config.schema import WebFetchConfig
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("hczkbot.security.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_can_skip_jina_and_use_custom_user_agent(monkeypatch):
    ...
    monkeypatch.setattr("hczkbot.agent.tools.web.httpx.AsyncClient", FakeClient)
...
async def test_web_fetch_blocks_private_redirect_before_readability_request(monkeypatch):
    ...
    with patch("hczkbot.security.network.socket.getaddrinfo", resolve_public_start_only):
...
async def test_web_fetch_blocks_private_redirect_before_returning_image(monkeypatch):
    ...
    monkeypatch.setattr("hczkbot.agent.tools.web.httpx.AsyncClient", TransportAsyncClient)

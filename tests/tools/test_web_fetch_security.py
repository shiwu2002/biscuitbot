...
from biscuitbot.agent.tools import web as web_module
from biscuitbot.agent.tools.web import WebFetchTool
from biscuitbot.config.schema import WebFetchConfig
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("biscuitbot.security.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_result_contains_untrusted_flag():
    ...
    with patch("biscuitbot.security.network.socket.getaddrinfo", _fake_resolve_public), \
...
async def test_web_fetch_can_skip_jina_and_use_custom_user_agent(monkeypatch):
    tool = WebFetchTool(
        ...
        user_agent="biscuitbot-test-agent",
    )
    ...
    monkeypatch.setattr("biscuitbot.agent.tools.web.httpx.AsyncClient", FakeClient)
...
async def test_web_fetch_blocks_private_redirect_before_readability_request(monkeypatch):
    ...
    with patch("biscuitbot.security.network.socket.getaddrinfo", resolve_public_start_only):
...
async def test_web_fetch_blocks_private_redirect_before_returning_image(monkeypatch):
    ...
    monkeypatch.setattr("biscuitbot.agent.tools.web.httpx.AsyncClient", TransportAsyncClient)

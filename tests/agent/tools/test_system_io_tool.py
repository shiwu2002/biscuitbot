...
from xianaibot.agent.tools.system_io import (
    _ALL_ACTIONS,
    SystemIoTool,
    SystemIoToolConfig,
)
...
class TestSystemIoToolMetadata:
    ...
    def test_docs_file_exists(self):
        ...
        docs_path = Path(__file__).resolve().parents[3] / "xianaibot" / "agent" / "tools" / "docs" / "system_io.md"
...
class TestSystemIoToolHappyPaths:
    ...
    async def test_key_tap_via_xdotool_backend(self, monkeypatch):
        ...
        import xianaibot.agent.tools.system_io as mod
...
class TestSystemIoConfigIntegration:
    ...
    def test_system_io_config_reexported_from_schema(self):
        from xianaibot.config.schema import SystemIoToolConfig as Reexported

...
def _make_loop(tmp_path):
    ...
    with patch("hczkbot.agent.loop.ContextBuilder"), \
         ...
         patch("hczkbot.agent.loop.SubagentManager") as MockSubMgr:
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("hczkbot.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "hczkbot.agent.runner.estimate_message_tokens",
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from hczkbot.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
def test_drop_orphan_tool_results_removes_unmatched_tool_messages():
    from hczkbot.agent.runner import AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from hczkbot.agent.loop import AgentLoop
    from hczkbot.agent.runner import _BACKFILL_CONTENT
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from hczkbot.agent.runner import AgentRunSpec, AgentRunner, _BACKFILL_CONTENT
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from hczkbot.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
...
def test_snip_history_preserves_user_message_after_truncation(monkeypatch):
    ...
    monkeypatch.setattr("hczkbot.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (500, None))

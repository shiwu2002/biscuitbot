...
def _make_loop(tmp_path):
    ...
    with patch("xianaibot.agent.loop.ContextBuilder"), \
         ...
         patch("xianaibot.agent.loop.SubagentManager") as mock_sub_mgr:
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    from xianaibot.agent.runner import _MAX_INJECTIONS_PER_TURN, AgentRunner, AgentRunSpec
    from xianaibot.bus.events import InboundMessage
...
async def test_injection_cycles_capped_at_max():
    ...
    from xianaibot.agent.runner import _MAX_INJECTION_CYCLES, AgentRunner, AgentRunSpec
    from xianaibot.bus.events import InboundMessage
...
async def test_cron_turn_deferred_while_session_active(tmp_path):
    ...
    from xianaibot.bus.events import InboundMessage
    from xianaibot.cron.session_turns import (
...
async def test_submitted_cron_turn_reports_pending_until_completed(tmp_path):
    ...
    from xianaibot.cron.session_turns import CRON_TRIGGER_META
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from xianaibot.agent.loop import AgentLoop
    from xianaibot.agent.runner import _MAX_INJECTIONS_PER_TURN
    from xianaibot.bus.events import InboundMessage
    from xianaibot.bus.queue import MessageBus
...
async def test_drain_injections_on_empty_final_response():
    ...
    from xianaibot.agent.runner import _MAX_EMPTY_RETRIES, AgentRunner, AgentRunSpec
    from xianaibot.bus.events import InboundMessage

...
try:
    from biscuitbot.channels import dingtalk
...
import biscuitbot.channels.dingtalk as dingtalk_module
from biscuitbot.bus.queue import MessageBus
from biscuitbot.channels.dingtalk import DingTalkChannel, DingTalkConfig, BiscuitbotDingTalkHandler
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "Biscuitbot Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = BiscuitbotDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/biscuitbot_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/biscuitbot_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "biscuitbot.config.paths.get_media_dir",

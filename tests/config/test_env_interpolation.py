...
from xianaibot.config.loader import (
    _resolve_env_vars,
    load_config,
...
class TestResolveConfig:
    ...
    def test_preserves_excluded_fields_when_no_env_refs(self, tmp_path):
        """Regression: extra provider fields (e.g. a custom OpenAI-compatible
        provider) must survive ``resolve_config_env_vars`` when the config has
        no ``${VAR}`` references. Previously the unconditional dump→revalidate
        roundtrip silently dropped them."""
        config_path = tmp_path / "config.json"
        config_path.write_text(
            json.dumps(
                {"providers": {"mycustom": {"apiKey": "secret"}}}
            ),
            encoding="utf-8",
        )

        raw = load_config(config_path)
        assert raw.providers.mycustom.api_key == "secret"

        resolved = resolve_config_env_vars(raw)
        assert resolved.providers.mycustom.api_key == "secret"

    def test_preserves_excluded_fields_with_env_refs(self, tmp_path, monkeypatch):
        """Extra provider fields must also survive when the config contains
        ``${VAR}`` refs elsewhere. An in-place walk preserves the extra

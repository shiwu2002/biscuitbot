...
def test_onboard_does_not_crash_with_legacy_memory_window(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("hczkbot.config.loader.get_config_path", lambda: config_path)
    monkeypatch.setattr("hczkbot.cli.commands.get_workspace_path", lambda _workspace=None: workspace)

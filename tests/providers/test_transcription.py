...
from xianaibot.audio.transcription import (
...
from xianaibot.providers.transcription import (
...
def test_resolver_uses_legacy_channel_provider_when_top_level_is_unset() -> None:
    ...
    assert resolved.api_base == "https://proxy.example"
...
def test_audio_format_maps_known_extensions() -> None:
    assert _audio_mime_type(Path("v.ogg")) == "audio/ogg"  # Telegram voice notes
    assert _audio_mime_type(Path("v.opus")) == "audio/ogg"
...
def test_resolve_chat_completions_url_appends_path_to_base() -> None:
    ...
    assert _resolve_api_path(None, default, path) == "https://api.xiaomimimo.com/v1/chat/completions"
    assert (
        _resolve_api_path("https://api.xiaomimimo.com/v1", default, path)

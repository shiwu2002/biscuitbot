...
from hczkbot.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with patch("hczkbot.cli.commands._PROMPT_SESSION", mock_session), \
         patch("hczkbot.cli.commands.patch_stdout"):
        yield mock_session
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("hczkbot.cli.commands._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:

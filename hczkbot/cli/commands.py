...
@app.command()
def platform(
    port: int | None = typer.Option(None, "--port", "-p", help="平台集成服务端口"),
    host: str | None = typer.Option(None, "--host", "-H", help="绑定地址"),
    timeout: float | None = typer.Option(None, "--timeout", "-t", help="单次请求超时（秒）"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="显示运行时日志"),
    ...
    同时提供：
    - 平台规范端点（/api/health, /api/chat, /api/chat/stream, /api/agent/info 等）
    - OpenAI 兼容端点（/v1/chat/completions）

    需在配置文件中设置 platform.enabled=true，或通过环境变量
    JWT_AGENT_SHARED_SECRET 注入共享密钥。
    """
    try:
        from aiohttp import web  # noqa: F401
    except ImportError:
    ...
    from hczkbot.api.server import attach_platform_routes, create_app
    from hczkbot.bus.queue import MessageBus
    from hczkbot.providers.image_generation import image_gen_provider_configs
    from hczkbot.session.manager import SessionManager
    ...
    runtime_config = _load_runtime_config(config, workspace)
    platform_cfg = runtime_config.platform

    if not platform_cfg.enabled:
        console.print(
            "[yellow]警告:[/yellow] platform.enabled=false，平台端点仍会启动但建议在配置中启用。\n"
            "  请在 ~/.hczkbot/config.json 中设置 {\"platform\": {\"enabled\": true}}"
        )
    ...
    model_name, preset_tag = _model_display(runtime_config)
    console.print(f"{__logo__} 启动桓宸智科 AI 平台集成模式")
    console.print(f"  [cyan]平台端点[/cyan]: http://{host}:{port}/api/health")
    console.print(f"  [cyan]对话接口[/cyan]: http://{host}:{port}/api/chat")
    console.print(f"  [cyan]流式对话[/cyan]: http://{host}:{port}/api/chat/stream")
    console.print(f"  [cyan]元信息  [/cyan]: http://{host}:{port}/api/agent/info")
    console.print(f"  [cyan]OpenAI  [/cyan]: http://{host}:{port}/v1/chat/completions")
    console.print(f"  [cyan]模型    [/cyan]: {model_name}{preset_tag}")
    console.print(f"  [cyan]超时    [/cyan]: {timeout}s")
    console.print(f"  [cyan]JWT签发者[/cyan]: {platform_cfg.jwt_issuer}")
    if host in {"0.0.0.0", "::"}:
        console.print(
            "[yellow]警告:[/yellow] 服务绑定到所有接口。"
            "请确保在可信网络边界、防火墙或反向代理之后。"
        )
    console.print()

    api_app = create_app(agent_loop, model_name=model_name, request_timeout=timeout)
    # 挂载平台规范路由
    attach_platform_routes(api_app, platform_cfg, workspace=str(runtime_config.workspace_path))

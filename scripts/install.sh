...
main_source="https://github.com/HKUDS/xianaibot/archive/refs/heads/main.zip"
...
install_failure_hint() {
  printf '%s\n' "Error: could not install xianaibot from $install_source." >&2
  ...
  printf '  %s\n' "$python_bin -m venv ~/.xianaibot/venv" >&2
  printf '  %s\n' "~/.xianaibot/venv/bin/python -m pip install --upgrade $install_target" >&2
  printf '%s\n' "Then start setup with:" >&2
  printf '  %s\n' "xianaibot onboard --wizard" >&2
  exit 1
}
...
By default this installs or upgrades xianaibot-ai from PyPI.
...
run_xianaibot() {
  case "$xianaibot_runner" in
    uv)
      uv tool run --from "$install_target" xianaibot "$@"
      ;;
    pipx)
      pipx run --spec "$install_target" xianaibot "$@"
      ;;
    python)
      "$xianaibot_python" -m xianaibot "$@"
      ;;
    *)
...
install_with_active_python() {
  ...
  xianaibot_runner="python"
  xianaibot_python="$python_bin"
}

install_with_uv() {
  info "Installing or upgrading xianaibot from $install_source with uv tool..."
  uv tool install --python "$python_bin" --force --upgrade "$install_target" || return 1
  xianaibot_runner="uv"
}

install_with_pipx() {
  info "Installing or upgrading xianaibot from $install_source with pipx..."
  pipx install --python "$python_bin" --force "$install_target" || return 1
  xianaibot_runner="pipx"
}

write_managed_wrapper() {
  bin_dir="${XIANAIBOT_BIN_DIR:-$HOME/.local/bin}"
  wrapper="$bin_dir/xianaibot"
  mkdir -p "$bin_dir" || return 0

  if [ -e "$wrapper" ] && ! grep -q "Generated by xianaibot installer" "$wrapper" 2>/dev/null; then
  ...
  cat > "$wrapper" <<EOF
#!/bin/sh
# Generated by xianaibot installer.
exec "$xianaibot_python" -m xianaibot "\$@"
EOF
  chmod +x "$wrapper" || return 0

  if ! command -v xianaibot >/dev/null 2>&1; then
    info "Installed a xianaibot launcher at $wrapper."
    info "Add $bin_dir to PATH to run xianaibot directly."
  fi
}
...
install_with_managed_venv() {
  ...
  venv_dir="${XIANAIBOT_VENV:-$HOME/.xianaibot/venv}"
  ...
  "$venv_python" - <<'PY' >/dev/null 2>&1 || fail "The managed venv uses Python older than 3.11. Remove it or set XIANAIBOT_VENV to a new path."
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY

  info "Installing or upgrading xianaibot from $install_source in $venv_dir..."
  ensure_pip "$venv_python" || return 1
  "$venv_python" -m pip install --upgrade "$install_target" || return 1

  xianaibot_runner="python"
  xianaibot_python="$venv_python"
  write_managed_wrapper
}
...
if [ -n "$python_bin" ]; then
  ...
  "$python_bin" - <<'PY' >/dev/null 2>&1 || fail "xianaibot requires Python 3.11 or newer"
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY
else
...
if [ "$dry_run" = "1" ]; then
  info "Dry run: would install or upgrade xianaibot from $install_source."
  if python_is_virtual_env; then
    ...
    info "Dry run: would run xianaibot as: $python_bin -m xianaibot"
  elif command -v uv >/dev/null 2>&1; then
    info "Dry run: would run: uv tool install --python $python_bin --force --upgrade $install_target"
    info "Dry run: would run xianaibot as: uv tool run --from $install_target xianaibot"
  elif command -v pipx >/dev/null 2>&1; then
    info "Dry run: would run: pipx install --python $python_bin --force $install_target"
    info "Dry run: would run xianaibot as: pipx run --spec $install_target xianaibot"
  else
    venv_dir="${XIANAIBOT_VENV:-$HOME/.xianaibot/venv}"
    ...
    info "Dry run: would run xianaibot as: $venv_dir/bin/python -m xianaibot"
  fi
  if [ "${XIANAIBOT_SKIP_WIZARD:-}" = "1" ]; then
    info "Dry run: would skip setup wizard because XIANAIBOT_SKIP_WIZARD=1."
  else
...
info "Installed xianaibot:"
run_xianaibot --version

if [ "${XIANAIBOT_SKIP_WIZARD:-}" = "1" ]; then
  info "Skipping setup wizard because XIANAIBOT_SKIP_WIZARD=1."
  info "Run this later: $(xianaibot_try_command) onboard --wizard"
  exit 0
fi

info "Starting setup wizard..."
run_xianaibot onboard --wizard

info "Done. Try: $(xianaibot_try_command) agent -m \"Hello!\""

...
main_source="https://github.com/HKUDS/biscuitbot/archive/refs/heads/main.zip"
...
install_failure_hint() {
  printf '%s\n' "Error: could not install biscuitbot from $install_source." >&2
  ...
  printf '  %s\n' "$python_bin -m venv ~/.biscuitbot/venv" >&2
  printf '  %s\n' "~/.biscuitbot/venv/bin/python -m pip install --upgrade $install_target" >&2
  printf '%s\n' "Then start setup with:" >&2
  printf '  %s\n' "biscuitbot onboard --wizard" >&2
  exit 1
}
...
By default this installs or upgrades biscuitbot-ai from PyPI.
...
run_biscuitbot() {
  case "$biscuitbot_runner" in
    uv)
      uv tool run --from "$install_target" biscuitbot "$@"
      ;;
    pipx)
      pipx run --spec "$install_target" biscuitbot "$@"
      ;;
    python)
      "$biscuitbot_python" -m biscuitbot "$@"
      ;;
    *)
...
install_with_active_python() {
  ...
  biscuitbot_runner="python"
  biscuitbot_python="$python_bin"
}

install_with_uv() {
  info "Installing or upgrading biscuitbot from $install_source with uv tool..."
  uv tool install --python "$python_bin" --force --upgrade "$install_target" || return 1
  biscuitbot_runner="uv"
}

install_with_pipx() {
  info "Installing or upgrading biscuitbot from $install_source with pipx..."
  pipx install --python "$python_bin" --force "$install_target" || return 1
  biscuitbot_runner="pipx"
}

write_managed_wrapper() {
  bin_dir="${BISCUITBOT_BIN_DIR:-$HOME/.local/bin}"
  wrapper="$bin_dir/biscuitbot"
  mkdir -p "$bin_dir" || return 0

  if [ -e "$wrapper" ] && ! grep -q "Generated by biscuitbot installer" "$wrapper" 2>/dev/null; then
  ...
  cat > "$wrapper" <<EOF
#!/bin/sh
# Generated by biscuitbot installer.
exec "$biscuitbot_python" -m biscuitbot "\$@"
EOF
  chmod +x "$wrapper" || return 0

  if ! command -v biscuitbot >/dev/null 2>&1; then
    info "Installed a biscuitbot launcher at $wrapper."
    info "Add $bin_dir to PATH to run biscuitbot directly."
  fi
}
...
install_with_managed_venv() {
  ...
  venv_dir="${BISCUITBOT_VENV:-$HOME/.biscuitbot/venv}"
  ...
  "$venv_python" - <<'PY' >/dev/null 2>&1 || fail "The managed venv uses Python older than 3.11. Remove it or set BISCUITBOT_VENV to a new path."
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY

  info "Installing or upgrading biscuitbot from $install_source in $venv_dir..."
  ensure_pip "$venv_python" || return 1
  "$venv_python" -m pip install --upgrade "$install_target" || return 1

  biscuitbot_runner="python"
  biscuitbot_python="$venv_python"
  write_managed_wrapper
}
...
if [ -n "$python_bin" ]; then
  ...
  "$python_bin" - <<'PY' >/dev/null 2>&1 || fail "biscuitbot requires Python 3.11 or newer"
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY
else
...
if [ "$dry_run" = "1" ]; then
  info "Dry run: would install or upgrade biscuitbot from $install_source."
  if python_is_virtual_env; then
    ...
    info "Dry run: would run biscuitbot as: $python_bin -m biscuitbot"
  elif command -v uv >/dev/null 2>&1; then
    info "Dry run: would run: uv tool install --python $python_bin --force --upgrade $install_target"
    info "Dry run: would run biscuitbot as: uv tool run --from $install_target biscuitbot"
  elif command -v pipx >/dev/null 2>&1; then
    info "Dry run: would run: pipx install --python $python_bin --force $install_target"
    info "Dry run: would run biscuitbot as: pipx run --spec $install_target biscuitbot"
  else
    venv_dir="${BISCUITBOT_VENV:-$HOME/.biscuitbot/venv}"
    ...
    info "Dry run: would run biscuitbot as: $venv_dir/bin/python -m biscuitbot"
  fi
  if [ "${BISCUITBOT_SKIP_WIZARD:-}" = "1" ]; then
    info "Dry run: would skip setup wizard because BISCUITBOT_SKIP_WIZARD=1."
  else
...
info "Installed biscuitbot:"
run_biscuitbot --version

if [ "${BISCUITBOT_SKIP_WIZARD:-}" = "1" ]; then
  info "Skipping setup wizard because BISCUITBOT_SKIP_WIZARD=1."
  info "Run this later: $(biscuitbot_try_command) onboard --wizard"
  exit 0
fi

info "Starting setup wizard..."
run_biscuitbot onboard --wizard

info "Done. Try: $(biscuitbot_try_command) agent -m \"Hello!\""

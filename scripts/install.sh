...
package="hczkbot-ai"
main_source="https://github.com/HKUDS/hczkbot/archive/refs/heads/main.zip"
...
install_failure_hint() {
  printf '%s\n' "Error: could not install hczkbot from $install_source." >&2
  ...
  printf '  %s\n' "$python_bin -m venv ~/.hczkbot/venv" >&2
  printf '  %s\n' "~/.hczkbot/venv/bin/python -m pip install --upgrade $install_target" >&2
  printf '%s\n' "Then start setup with:" >&2
  printf '  %s\n' "hczkbot onboard --wizard" >&2
  exit 1
}
...
By default this installs or upgrades hczkbot-ai from PyPI.
...
run_hczkbot() {
  case "$hczkbot_runner" in
    uv)
      uv tool run --from "$install_target" hczkbot "$@"
      ;;
    pipx)
      pipx run --spec "$install_target" hczkbot "$@"
      ;;
    python)
      "$hczkbot_python" -m hczkbot "$@"
      ;;
    *)
...
install_with_active_python() {
  ...
  hczkbot_runner="python"
  hczkbot_python="$python_bin"
}

install_with_uv() {
  info "Installing or upgrading hczkbot from $install_source with uv tool..."
  uv tool install --python "$python_bin" --force --upgrade "$install_target" || return 1
  hczkbot_runner="uv"
}

install_with_pipx() {
  info "Installing or upgrading hczkbot from $install_source with pipx..."
  pipx install --python "$python_bin" --force "$install_target" || return 1
  hczkbot_runner="pipx"
}

write_managed_wrapper() {
  bin_dir="${HCZKBOT_BIN_DIR:-$HOME/.local/bin}"
  wrapper="$bin_dir/hczkbot"
  mkdir -p "$bin_dir" || return 0

  if [ -e "$wrapper" ] && ! grep -q "Generated by hczkbot installer" "$wrapper" 2>/dev/null; then
  ...
  cat > "$wrapper" <<EOF
#!/bin/sh
# Generated by hczkbot installer.
exec "$hczkbot_python" -m hczkbot "\$@"
EOF
  chmod +x "$wrapper" || return 0

  if ! command -v hczkbot >/dev/null 2>&1; then
    info "Installed a hczkbot launcher at $wrapper."
    info "Add $bin_dir to PATH to run hczkbot directly."
  fi
}
...
install_with_managed_venv() {
  ...
  venv_dir="${HCZKBOT_VENV:-$HOME/.hczkbot/venv}"
  ...
  "$venv_python" - <<'PY' >/dev/null 2>&1 || fail "The managed venv uses Python older than 3.11. Remove it or set HCZKBOT_VENV to a new path."
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY

  info "Installing or upgrading hczkbot from $install_source in $venv_dir..."
  ensure_pip "$venv_python" || return 1
  "$venv_python" -m pip install --upgrade "$install_target" || return 1

  hczkbot_runner="python"
  hczkbot_python="$venv_python"
  write_managed_wrapper
}
...
if [ -n "$python_bin" ]; then
  ...
  "$python_bin" - <<'PY' >/dev/null 2>&1 || fail "hczkbot requires Python 3.11 or newer"
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY
else
...
if [ "$dry_run" = "1" ]; then
  info "Dry run: would install or upgrade hczkbot from $install_source."
  if python_is_virtual_env; then
    ...
    info "Dry run: would run hczkbot as: $python_bin -m hczkbot"
  elif command -v uv >/dev/null 2>&1; then
    info "Dry run: would run: uv tool install --python $python_bin --force --upgrade $install_target"
    info "Dry run: would run hczkbot as: uv tool run --from $install_target hczkbot"
  elif command -v pipx >/dev/null 2>&1; then
    info "Dry run: would run: pipx install --python $python_bin --force $install_target"
    info "Dry run: would run hczkbot as: pipx run --spec $install_target hczkbot"
  else
    venv_dir="${HCZKBOT_VENV:-$HOME/.hczkbot/venv}"
    ...
    info "Dry run: would run hczkbot as: $venv_dir/bin/python -m hczkbot"
  fi
  if [ "${HCZKBOT_SKIP_WIZARD:-}" = "1" ]; then
    info "Dry run: would skip setup wizard because HCZKBOT_SKIP_WIZARD=1."
  else
...
info "Installed hczkbot:"
run_hczkbot --version

if [ "${HCZKBOT_SKIP_WIZARD:-}" = "1" ]; then
  info "Skipping setup wizard because HCZKBOT_SKIP_WIZARD=1."
  info "Run this later: $(hczkbot_try_command) onboard --wizard"
  exit 0
fi

info "Starting setup wizard..."
run_hczkbot onboard --wizard

info "Done. Try: $(hczkbot_try_command) agent -m \"Hello!\""

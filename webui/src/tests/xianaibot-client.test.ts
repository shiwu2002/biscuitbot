...
/**
 * Minimal fake WebSocket implementing the subset XianaibotClient touches.
...
describe("XianaibotClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("xianaibot-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("xianaibot-host://engine/");
    expect(lastSocket().url).toBe("host:xianaibot-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    Object.defineProperty(window, "xianaibotHost", {
      configurable: true,
      value: {
        openSocket,

...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "xianaibot-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('xianaibot_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('xianaibot_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("xianaibot_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface XianaibotClientOptions {
  url: string;
  reconnect?: boolean;
...
export class XianaibotClient {
  ...
  constructor(private options: XianaibotClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[xianaibot ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[xianaibot ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - XianaibotClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }

"""OpenAI-compatible HTTP API server for a fixed xianaibot session.
...
from xianaibot.utils.runtime import EMPTY_FINAL_RESPONSE_MESSAGE
...
async def handle_chat_completions(request: web.Request) -> web.StreamResponse:
    ...
    model_name: str = request.app.get("model_name", "xianaibot")
...
def _collect_models(config: Any, model_name: str) -> list[dict[str, Any]]:
    ...
    # Chat：当前激活模型 + 所有命名预设
    add(model_name, "xianaibot")
    for preset in getattr(config, "model_presets", {}).values():
        add(getattr(preset, "model", None), "xianaibot")
    ...
    if tts_cfg is not None and getattr(tts_cfg, "enabled", True):
        try:
            ...
            add(effective.model, effective.provider or "xianaibot-tts")
        except Exception:
            add(getattr(tts_cfg, "model", None), "xianaibot-tts")
    ...
    if transcription_cfg is not None and getattr(transcription_cfg, "enabled", False):
        add(
            ...
            getattr(transcription_cfg, "provider", None) or "xianaibot-transcription",
        )
...
async def handle_models(request: web.Request) -> web.Response:
    ...
    if models is None:
        model_name = request.app.get("model_name", "xianaibot")
        models = [
            {"id": model_name, "object": "model", "created": 0, "owned_by": "xianaibot"}
        ]

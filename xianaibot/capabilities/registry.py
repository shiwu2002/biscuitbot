...
from xianaibot.apps.protocol import capability_manifest
...
# 其余内置技能只是带 bins/env 依赖的指令，仍为 prompt。
# 办公类技能（docx/pptx/pdf/xlsx）捆绑 OOXML 校验、表单填充、
...
class CapabilityRegistry:
    ...
    def _cli_manager(self) -> Any:
        if self._cli is not None:
            return self._cli
        from xianaibot.apps.cli import CliAppManager, CliAppsRuntimeConfig
    ...
    def _mcp_capabilities(self) -> list[dict[str, Any]]:
        try:
            from xianaibot.webui.mcp_presets_api import mcp_presets_payload

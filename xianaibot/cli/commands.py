...
所属模块与项目作用
===================
本文件位于 xianaibot/cli 目录，是 CLI 模块的核心命令实现文件。
在项目架构中起到的作用：
- 定义 xianaibot 命令行工具的所有子命令（onboard、agent、gateway、
...
# 移除默认处理器，并以统一的 xianaibot 格式重新添加
logger.remove()
...
from xianaibot import __logo__, __version__  # noqa: E402  # 项目 logo 与版本号
from xianaibot.agent.loop import AgentLoop  # noqa: E402  # 智能体主循环
from xianaibot.cli.stream import StreamRenderer, ThinkingSpinner  # noqa: E402  # 流式渲染与思考 spinner
from xianaibot.config.paths import get_workspace_path, is_default_workspace  # noqa: E402  # 工作区路径工具
from xianaibot.config.schema import Config  # noqa: E402  # 配置 schema
from xianaibot.utils.evaluator import evaluate_response  # noqa: E402  # 响应评估器（用于心跳）
from xianaibot.utils.helpers import sync_workspace_templates  # noqa: E402  # 工作区模板同步
from xianaibot.utils.restart import (  # noqa: E402  # 重启通知工具
...
def _init_prompt_session() -> None:
    ...
    from xianaibot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} 夏奈儿[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} 夏奈儿[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} 夏奈儿 v{__version__}")
        raise typer.Exit()
...
def _has_any_api_key(config) -> bool:
    ...
    from xianaibot.providers.registry import PROVIDERS
...
def _run_quick_setup(config, config_path: Path) -> None:
    ...
    # --- Step 0: Welcome ---
    console.print()
    console.print(Align.center(f"{__logo__} [bold cyan]夏奈儿 快速安装[/bold cyan]"))
    ...
    console.print("    [green]xianaibot agent -m \"Hello!\"[/green]")
    console.print("    [green]xianaibot gateway[/green]")
    console.print()
    console.print("  更多选项（频道、预设、工具），请运行：")
    console.print("    [dim]xianaibot onboard --wizard[/dim]")
    console.print()
...
def onboard(
    ...
    """初始化 xianaibot 配置和工作区。"""
    from xianaibot.config.loader import get_config_path, load_config, save_config, set_config_path
    ...
    if wizard:
        from xianaibot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]请再次运行 'xianaibot onboard' 完成安装。[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'xianaibot agent -m "Hello!"'
    gateway_cmd = "xianaibot gateway"
    if config:
    ...
    if _has_any_api_key(config):
        ...
        console.print("  3. 高级设置：[cyan]xianaibot onboard --wizard[/cyan]")
    elif wizard:
        console.print("\n后续步骤：")
        console.print(f"  1. 聊天：[cyan]{agent_cmd}[/cyan]")
    ...
    console.print(
        "\n[dim]Docs: https://github.com/xianaibot/xianaibot[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from xianaibot.channels.registry import discover_all
    from xianaibot.config.loader import _load_config_file
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from xianaibot.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from xianaibot.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from xianaibot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="显示夏奈儿运行时日志"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="工作区目录"),
    config_file: str | None = typer.Option(None, "--config", "-c", help="配置文件路径"),
):
    ...
    try:
        ...
        console.print("[red]需要 aiohttp。请安装：pip install 'xianaibot[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from xianaibot.api.server import create_app
    from xianaibot.bus.queue import MessageBus
    from xianaibot.providers.image_generation import image_gen_provider_configs
    from xianaibot.session.manager import SessionManager

    if verbose:
        logger.enable("xianaibot")
    else:
        logger.disable("xianaibot")
...
def _run_gateway(
    ...
    *,
    ...
    会自动热替换为真实 Provider。普通 ``xianaibot gateway`` 保持快速失败。
    """
    ...
    from xianaibot.cron.bound_runner import run_bound_cron_job
    from xianaibot.cron.service import CronJobSkippedError, CronService
    from xianaibot.cron.session_turns import is_bound_cron_job
    from xianaibot.cron.types import CronJob
    ...
    from xianaibot.webui.token_usage import TokenUsageHook

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} 正在启动夏奈儿网关，版本 {__version__} 端口 {port}...")
    if open_browser_url:
        console.print(f"  WebUI 地址：[cyan]{open_browser_url}[/cyan]")
    sync_workspace_templates(config.workspace_path)
    from xianaibot.config.paths import migrate_legacy_channel_media
    ...
    from xianaibot.bus.trace_logger import install_trace_logger
    install_trace_logger(runtime_events)

    from xianaibot.bus.events import OutboundMessage
    from xianaibot.session.keys import session_key_for_channel
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.name == "dream":
            from xianaibot.agent.memory import MemoryStore
            ...
            last_resp = None
            batches_processed = 0
            from xianaibot.command.builtin import _DREAM_BATCH_TIMEOUT_S, _DREAM_MAX_BATCHES
            try:
                ...
                from xianaibot.webui.token_usage import record_response_token_usage
        ...
        if job.name == "nightly_maintenance":
            ...
            from xianaibot.agent.tools.docs_consistency import (
                build_repair_task,
...
def sidecar(
    ...
    from xianaibot.desktop.sidecar import main as sidecar_main
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="在聊天中显示夏奈儿运行时日志"),
):
    """直接与智能体交互。"""
    from loguru import logger
...
def channels_status(
    ...
    """显示频道状态。"""
    from xianaibot.channels.registry import discover_all
    from xianaibot.config.loader import load_config, set_config_path
...
def plugins_list():
    """列出所有已发现的频道（内置和插件）。"""
    from xianaibot.channels.registry import discover_all, discover_channel_names
...
def status():
    """显示 xianaibot 状态。"""
    from xianaibot.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} 夏奈儿 Status\n")
...
def _resolve_talent_config_path(config_path: str | None) -> Path:
    ...
    from xianaibot.config.loader import get_config_path, set_config_path
...
def _load_talent_config_raw(path: Path) -> dict:
    """读取配置文件：缺失返回 {}；不可读则报错退出（Exit 1）。"""
    import json as _json
...
def _drop_talent_single_keys(gateway: dict) -> None:
    """删除旧单值键（``talent_market_registry_url``/``talentMarketRegistryUrl``），
    使列表成为唯一权威形态。"""
    gateway.pop("talent_market_registry_url", None)
    gateway.pop("talentMarketRegistryUrl", None)
...
def talent_market_set(
    ...
    """整体替换人才市场注册表 URL 列表（写入后台配置文件；WebUI/桌面应用只读，不可修改）。"""
    from xianaibot.webui.talent_market import TalentMarketError, _validate_registry_url
    ...
    raw = _load_talent_config_raw(path)
    gateway = raw.get("gateway")
    if not isinstance(gateway, dict):
        gateway = {}
        raw["gateway"] = gateway
    # 手术式写入：仅改人才市场相关键，绝不触碰配置文件里的其他键（含 API Key）
    gateway["talent_market_registry_urls"] = [url]
    _drop_talent_single_keys(gateway)
    _write_talent_config(path, raw)
    console.print(f"[green]已设置人才市场注册表：[/green]{url}")
    console.print(f"[dim]配置文件：{path}[/dim]")


@talent_market_app.command("add")
def talent_market_add(
    url: str,
    config_path: str | None = typer.Option(None, "--config", "-c", help="配置文件路径"),
):
    """追加一个人才市场注册表 URL（幂等，保留既有来源）。"""
    ...
    if not isinstance(gateway, dict):
        gateway = {}
        raw["gateway"] = gateway
    urls = _talent_urls_from_gateway(gateway)
    if url in urls:
        console.print(f"人才市场注册表已存在：{url}")
        console.print(f"[dim]配置文件：{path}[/dim]")
        return
    urls.append(url)
    gateway["talent_market_registry_urls"] = urls
    _drop_talent_single_keys(gateway)
    _write_talent_config(path, raw)
    console.print(f"[green]已添加人才市场注册表：[/green]{url}")
    console.print(f"[dim]配置文件：{path}[/dim]")


@talent_market_app.command("remove")
def talent_market_remove(
    url: str,
    config_path: str | None = typer.Option(None, "--config", "-c", help="配置文件路径"),
):
    """按精确 URL 删除一个人才市场注册表来源。"""
    from xianaibot.webui.talent_market import _talent_urls_from_gateway
    ...
    if urls:
        ...
        gateway.pop("talent_market_registry_urls", None)
        gateway.pop("talentMarketRegistryUrls", None)
    _drop_talent_single_keys(gateway)
    _write_talent_config(path, raw)
    console.print(f"[green]已删除人才市场注册表：[/green]{url}")
    console.print(f"[dim]配置文件：{path}[/dim]")


@talent_market_app.command("list")
def talent_market_list(
    config_path: str | None = typer.Option(None, "--config", "-c", help="配置文件路径"),
):
    """列出全部人才市场注册表 URL。"""
    from xianaibot.webui.talent_market import read_talent_market_registry_urls

    path = _resolve_talent_config_path(config_path)
    urls = read_talent_market_registry_urls(path)
    if urls:
        ...
        console.print("人才市场注册表：[dim]未配置[/dim]")
        console.print(
            "提示：使用 `xianaibot talent-market add <url>`（追加）"
            "或 `set <url>`（整体替换）进行配置。"
        )
    console.print(f"[dim]配置文件：{path}[/dim]")
...
def talent_market_show(
    ...
    """显示当前全部人才市场注册表 URL（等价于 list）。"""
    talent_market_list(config_path)
...
def talent_market_clear(
    ...
    """清除全部人才市场注册表 URL 配置。"""
    from xianaibot.webui.talent_market import _TALENT_URL_KEYS
    ...
    if not isinstance(gateway, dict):
        ...
        console.print(f"[dim]配置文件：{path}[/dim]")
        return
    changed = any(k in gateway for k in _TALENT_URL_KEYS)
    for k in _TALENT_URL_KEYS:
        gateway.pop(k, None)
    if changed:
        _write_talent_config(path, raw)

...
所属模块与项目作用
===================
本文件位于 xianaibot/cli 目录，是 CLI 模块中的交互式配置引导组件。
在项目架构中起到的作用：
...
from xianaibot.cli.models import (  # 模型信息辅助工具
    format_token_count,
    get_model_context_limit,
    get_model_suggestions,
)
from xianaibot.config.loader import get_config_path, load_config  # 配置文件加载
from xianaibot.config.schema import Config, ModelPresetConfig  # 配置 schema 定义
...
def _show_main_menu_header() -> None:
    """展示主菜单头部。"""
    from xianaibot import __logo__, __version__
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]夏奈儿[{__version__}][/bold cyan]")
    )
    console.print()
...
def _handle_fallback_models_field(
    ...
    from xianaibot.config.schema import InlineFallbackConfig
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    注意:
        本函数会从 xianaibot.config.schema 导入 AgentDefaults，
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"xianaibot.channels.{name}")
...
def _configure_mcp_servers(config: Config) -> None:
    ...
    from xianaibot.webui.mcp_presets_api import MCP_PRESETS
...
def _add_mcp_custom(config: Config) -> None:
    """手动添加自定义 MCP 服务器。"""
    from xianaibot.config.schema import MCPServerConfig

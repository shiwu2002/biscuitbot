...
def _resolve_locked_skill_key(requested: str, skills: Dict[str, Any]) -> str:
    requested = str(requested or "").strip()
    if not requested or requested in skills:
        return requested
    for key, value in skills.items():
        meta = value if isinstance(value, dict) else {}
        candidates = {
            str(meta.get("publicSlug") or "").strip(),
            str(meta.get("internalSlug") or "").strip(),
            str(meta.get("slug") or "").strip(),
        }
...
def cmd_upgrade(args: Any, deps: Dict[str, Any]) -> int:
    ...
    if args.slug:
        targets = [_resolve_locked_skill_key(args.slug, skills)]
    else:
    ...
    for slug in targets:
        ...
        install_dir = _first_non_empty_string(lock_meta_dict, ["installDir", "targetDir"])
        target_dir = Path(install_dir).expanduser().resolve() if install_dir else install_root / slug

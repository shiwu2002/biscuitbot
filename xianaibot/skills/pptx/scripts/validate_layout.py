...
class PPTXLayoutValidator:
    ...
    def __init__(self, pptx_path: Optional[str] = None, verbose: bool = False, slide_filter: Optional[Set[int]] = None):
        ...
        self._master_default_font_size: int = 1800  # hundredths of a point, 18pt default
    ...
    def _estimate_cell_required_height_emu(self, tc_node, cell_width_emu: int) -> int:
        ...
        for tx_body in tx_bodies:
            ...
            for p in ps:
                ...
                max_sz = max(sz_values) if sz_values else self._master_default_font_size
    ...
    def _parse_master_default_font_size(self, zf: zipfile.ZipFile):
        """Parse default font size from slideMaster otherStyle."""
        master_files = [f for f in zf.namelist() if "slideMasters/slideMaster" in f and f.endswith(".xml")]
        for mf in master_files[:1]:
            try:
                master_xml = zf.read(mf)
                master_dom = defusedxml.minidom.parseString(master_xml)
                other_styles = master_dom.getElementsByTagName("p:otherStyle")
                if not other_styles:
                    continue
                lvl1 = other_styles[0].getElementsByTagName("a:lvl1pPr")
                if not lvl1:
                    continue
                def_rpr = lvl1[0].getElementsByTagName("a:defRPr")
                if not def_rpr:
                    continue
                if def_rpr[0].hasAttribute("sz"):
                    sz = int(def_rpr[0].getAttribute("sz"))
                    if sz > 0:
                        self._master_default_font_size = sz
            except Exception:
                pass
    ...
    def _extract_slide_info(self):
        ...
        with zipfile.ZipFile(self.pptx_path, "r") as zf:
            ...
            # Parse slideMaster default font size from otherStyle
            self._parse_master_default_font_size(zf)

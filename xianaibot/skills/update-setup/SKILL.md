...
name: update-setup
tier: system
description: "One-time setup wizard for the 夏奈儿 upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新."
---
...
Use `exec` to run `xianaibot --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v xianaibot || true
python -m pip show xianaibot-ai || true
pipx list | sed -n '/xianaibot-ai/,+3p' || true
uv tool list | sed -n '/xianaibot-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how 夏奈儿 was
installed.
...
`question: "Where is your 夏奈儿 source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "xianaibot-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "xianaibot-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "xianaibot-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade 夏奈儿 to the latest version. Triggers: upgrade 夏奈儿, update 夏奈儿, 升级夏奈儿, 更新夏奈儿."
---
...
4. Use `exec` to verify: `xianaibot --version`
5. Tell the user the new version. Say: "Run `/restart` to restart 夏奈儿 and apply the update. If `/restart` is unavailable in this channel, restart the 夏奈儿 process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade 夏奈儿' when you want to update."

...
1. **Confirm the tool is enabled.** It is off by default. If `system_io` is not registered, tell the user to set `tools.system_io.enable = true` in config. The pynput/pyserial dependencies are bundled with 夏奈儿, so no extra install is needed for full keyboard/mouse/serial support.

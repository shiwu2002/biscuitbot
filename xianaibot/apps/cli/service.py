...
from xianaibot.apps.protocol import capability_manifest, compact_dict
from xianaibot.config.paths import get_runtime_subdir
from xianaibot.security.workspace_policy import is_path_within

logger = logging.getLogger(__name__)

# CLI-Anything 注册表（港大 HKUDS 维护的开源 Agent CLI 应用目录）
# 来源: https://github.com/HKUDS/CLI-Anything
# 包含经过审核的应用条目（name, entry_point, install_cmd, skill_md 等）
CLI_ANYTHING_REGISTRY_URL = "https://hkuds.github.io/CLI-Anything/registry.json"

# CLI-Anything 原始文件基址（用于拼接 SKILL.md 等技能文件的下载 URL）
...
class CliAppManager:
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["xianaibot_state_absent", "managed_paths_absent"]
            ),
        })
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from 夏奈儿."
        return f"""---
...
Use this skill when the user asks 夏奈儿 to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_xianaibot_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- xianaibot-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_xianaibot_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from 夏奈儿. {entry_point} "
                "is still available because it is managed outside 夏奈儿."
            )

"""Utility functions for xianaibot."""
...
_TOOL_RESULTS_DIR = ".xianaibot/tool-results"
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("xianaibot") / "templates"
    except Exception:
    ...
    # 直接覆盖以保持与代码一致。
    try:
        docs_src = pkg_files("xianaibot") / "agent" / "tools" / "docs"
        if docs_src.is_dir():
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the xianaibot package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("xianaibot") / "templates" / template_name
        if tpl.is_file():

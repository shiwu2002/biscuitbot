"""xianaibot 的高层编程接口（SDK Facade 层）。

所属模块与项目作用
===================
本文件位于 ``xianaibot`` 包根目录，是整个项目的 **SDK 门面层（Facade）**，
...
典型用法::
    ...
    async with Xianaibot.from_config() as bot:
...
from xianaibot.agent.hook import AgentHook, SDKCaptureHook
# AgentLoop：项目核心状态机引擎，Xianaibot 是它的薄封装
...
class RunResult:
    ...
    由 ``Xianaibot.run()`` 返回，封装了 LLM 最终回复文本、本次调用的
...
class Xianaibot:
    """运行 xianaibot agent 的编程门面（facade）。
    ...
    async def __aenter__(self) -> Xianaibot:

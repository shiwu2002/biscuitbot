...
所属模块与项目作用
===================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的核心协调组件。
...
from xianaibot.channels.deps import (  # 渠道 SDK 依赖检测/自动安装
    channel_sdk_available,
    ensure_channel_deps,
)
from xianaibot.config.schema import Config  # 全局配置模型
from xianaibot.utils.restart import (  # 重启通知处理
    consume_restart_notice_from_env,
    format_restart_completed_message,
)

if TYPE_CHECKING:
    from xianaibot.session.manager import SessionManager  # 会话管理器（仅类型检查时导入）


def _default_webui_dist() -> Path | None:
    """返回内置 webui dist 目录的绝对路径（如存在）。"""
    try:
        import xianaibot.web as web_pkg  # type: ignore[import-not-found]
    except ImportError:
        return None
...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        from xianaibot.channels.registry import discover_channel_names, discover_enabled

...
所属模块与项目作用
===================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的抽象基类组件。
...
from xianaibot.bus.events import InboundMessage, OutboundMessage  # 消息总线事件类型（入站/出站）
from xianaibot.bus.queue import MessageBus  # 消息总线，渠道与核心之间的通信通道
from xianaibot.pairing import (  # 配对码模块：未授权用户首次私聊时下发一次性配对码
...
class BaseChannel(ABC):
    ...
    每个具体渠道（Telegram、Discord 等）应实现该接口，以便接入 xianaibot 的消息总线。

...
所属模块与项目作用
===================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的 QQ（Napcat）平台组件。
在项目架构中起到的作用：通过 OneBot v11 协议将 QQ 的消息收发能力接入 xianaibot 消息总线。
...
class NapcatChannel(BaseChannel):
    ...
    async def _build_image_segment(self, ref: str) -> dict[str, Any] | None:
        ...
        # different host/container than xianaibot.

...
所属模块与项目作用
====================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的 Mochat 平台组件。
在项目架构中起到的作用：通过 Socket.IO 长连接（或 HTTP 轮询降级）将 Mochat 平台的消息收发能力接入 xianaibot 消息总线。
...
from xianaibot.config.paths import get_runtime_subdir  # 运行时子目录
from xianaibot.config.schema import Base  # 配置模型基类
from xianaibot.utils.helpers import MessageIdDedup  # 有界消息 ID 去重器（共享工具）

...
所属模块与项目作用
===================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的邮件平台组件。
在项目架构中起到的作用：将电子邮件的收发能力接入 xianaibot 消息总线。
...
class EmailChannel(BaseChannel):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        base_subject = self._last_subject_by_chat.get(to_addr, "夏奈儿 reply")
    ...
    def _reply_subject(self, base_subject: str) -> str:
        ...
        subject = (base_subject or "").strip() or "夏奈儿 reply"

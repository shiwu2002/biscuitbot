...
所属模块与项目作用
===================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的 QQ 官方机器人平台组件。
在项目架构中起到的作用：通过 QQ botpy SDK 将 QQ 频道和群聊的消息收发能力接入 xianaibot 消息总线。
...
from xianaibot.bus.events import OutboundMessage  # 出站消息事件
...
from xianaibot.utils.helpers import MessageIdDedup, safe_filename  # 消息去重与文件名安全化共享工具
from xianaibot.utils.logging_bridge import redirect_lib_logging  # 第三方库日志桥接

try:
    from xianaibot.config.paths import get_media_dir  # 媒体文件目录
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # 禁用 botpy 的文件日志 —— xianaibot 使用 loguru；默认的 "botpy.log" 在只读文件系统上会失败
...
class QQConfig(Base):
    ...
    # 可选：入站附件保存目录。为空时使用 xianaibot 的 get_media_dir("channels/qq")
    media_dir: str = ""
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".xianaibot" / "media" / "channels" / "qq"
        else:
            root = Path.home() / ".xianaibot" / "media" / "channels" / "qq"

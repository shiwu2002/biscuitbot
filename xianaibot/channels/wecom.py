...
所属模块与项目作用
===================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的企业微信平台组件。
在项目架构中起到的作用：将企业微信 AI 机器人的消息收发能力接入 xianaibot 消息总线。
...
class WecomChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not WECOM_AVAILABLE:
            self.logger.error("SDK not installed. Run: pip install xianaibot[wecom]")
            return

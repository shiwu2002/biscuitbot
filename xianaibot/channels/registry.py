...
所属模块与项目作用
===================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的发现组件。
在项目架构中起到的作用：扫描并加载所有可用的渠道类，支持内置渠道模块的
自动发现和通过 entry_points 注册的外部插件加载。

平台特点与接入方式
------------------
- 内置渠道发现：通过 pkgutil.iter_modules 扫描 xianaibot.channels 包下的模块，
  零导入成本，仅返回模块名称列表。
- 外部插件发现：通过 importlib.metadata.entry_points 加载 ``xianaibot.channels``
  组下注册的第三方渠道插件。
...
if TYPE_CHECKING:
    from xianaibot.channels.base import BaseChannel  # 渠道抽象基类（仅类型检查时导入）
...
def discover_channel_names() -> list[str]:
    ...
    import xianaibot.channels as pkg
...
def load_channel_class(module_name: str) -> type[BaseChannel]:
    ...
    from xianaibot.channels.base import BaseChannel as _Base

    mod = importlib.import_module(f"xianaibot.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, _Base) and obj is not _Base:
            return obj
    raise ImportError(f"No BaseChannel subclass in xianaibot.channels.{module_name}")
...
def discover_plugins(enabled_names: set[str] | None = None) -> dict[str, type[BaseChannel]]:
    ...
    for ep in entry_points(group="xianaibot.channels"):

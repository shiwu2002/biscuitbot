...
所属模块与项目作用
===================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的钉钉平台组件。
在项目架构中起到的作用：将钉钉机器人的收发消息能力接入 xianaibot 消息总线。
...
from xianaibot.config.schema import Base
from xianaibot.security.network import validate_resolved_url, validate_url_target  # SSRF 防护校验
from xianaibot.utils.helpers import safe_filename  # 文件名安全化工具（防路径遍历）
...
class XianaibotDingTalkHandler(CallbackHandler):
    """钉钉 Stream SDK 标准回调处理器。

    解析入站消息并将其转发给 Xianaibot 的钉钉渠道（``DingTalkChannel``）处理。
    """
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            ...
            # 通过 _on_message 转发给 Xianaibot（非阻塞）。
...
class DingTalkChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # 注册标准回调处理器
            handler = XianaibotDingTalkHandler(self)
    ...
    async def _send_markdown_text(self, token: str, chat_id: str, content: str) -> bool:
        ...
        return await self._send_batch_message(
            ...
            chat_id,
            "sampleMarkdown",
            {"text": content, "title": "Xianaibot Reply"},
        )
    ...
    async def _on_message(
        ...
        """处理入站消息（由 ``XianaibotDingTalkHandler`` 调用）。

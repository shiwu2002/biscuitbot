...
所属模块与项目作用
==================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的个人微信平台组件。
在项目架构中起到的作用：通过 ilinkai.weixin.qq.com API 将个人微信的消息收发能力接入 xianaibot 消息总线。
...
from xianaibot.config.paths import get_media_dir, get_runtime_subdir  # 媒体目录与运行时子目录获取
from xianaibot.config.schema import Base  # 配置模型基类
from xianaibot.pairing import clear_channel  # 重新扫码登录后清空配对授权
from xianaibot.security.network import validate_url_target  # SSRF 防护校验（CDN 媒体下载）
from xianaibot.utils.helpers import MessageIdDedup, atomic_write_json, split_message  # 共享工具
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # 状态目录（默认：~/.xianaibot/weixin/）
...
class WeixinChannel(BaseChannel):
    ...
    async def _send_text(
        ...
        client_id = f"xianaibot-{uuid.uuid4().hex[:12]}"

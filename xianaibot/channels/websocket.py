"""WebSocket 服务端渠道：xianaibot 作为 WebSocket 服务端，为已连接的客户端提供服务。

所属模块与项目作用
==================
本文件位于 xianaibot/channels 目录，是 Channel（聊天平台接入）层的 WebSocket 平台组件。
在项目架构中起到的作用：作为 WebSocket 服务端运行，将 Web UI 及其他 WebSocket 客户端的消息收发能力接入 xianaibot 消息总线。

平台特点与接入方式
------------------
- 接入方式：xianaibot 自身作为 WebSocket 服务端，客户端通过 ws:// 或 wss:// 连接。
...
from xianaibot.bus.events import (  # 出站消息事件及 Agent UI 元数据键
    OUTBOUND_META_AGENT_UI,
    OutboundMessage,
)
from xianaibot.bus.queue import MessageBus  # 消息总线
from xianaibot.channels.base import BaseChannel  # 渠道抽象基类
from xianaibot.config.paths import get_media_dir  # 媒体目录获取
from xianaibot.config.schema import Base  # 配置模型基类
from xianaibot.security.workspace_access import (  # 工作区访问控制
    WORKSPACE_SCOPE_METADATA_KEY,  # 工作区作用域元数据键
    WorkspaceScopeError,  # 工作区作用域错误
)
from xianaibot.session.goal_state import goal_state_ws_blob  # 目标状态 WebSocket 数据块
from xianaibot.session.webui_turns import websocket_turn_wall_started_at  # WebUI 轮次启动时间
from xianaibot.utils.media_decode import (  # 媒体解码工具
    FileSizeExceeded,  # 文件大小超限异常
    save_base64_data_url,  # 保存 base64 data URL 为文件
)
from xianaibot.webui.cli_apps_api import normalize_cli_app_mentions  # CLI 应用提及归一化
from xianaibot.webui.forking import handle_webui_fork_chat  # 会话分叉处理
from xianaibot.webui.gateway_services import GatewayServices  # 网关服务集合
from xianaibot.webui.http_utils import (  # HTTP 工具函数
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``：若非空，对该路径的 **GET**（HTTP/1.1）请求返回 JSON
      ...
      必须与 ``path``（WS 升级路径）不同。若客户端与 xianaibot 运行在 **同一进程**
...
class WebSocketChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        """
        from xianaibot.utils.logging_bridge import redirect_lib_logging

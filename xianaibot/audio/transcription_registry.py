...
Provider-specific HTTP adapters live in ``xianaibot.providers.transcription``.
...
TRANSCRIPTION_PROVIDERS: tuple[TranscriptionProviderSpec, ...] = (
    TranscriptionProviderSpec(
        ...
        adapter="xianaibot.providers.transcription:OpenAITranscriptionProvider",
    ),
    TranscriptionProviderSpec(
        name="groq",
    ...
    TranscriptionProviderSpec(
        name="newapi",
        default_model="whisper-1",
        adapter="xianaibot.providers.transcription:NewApiTranscriptionProvider",
    ),
)
...
def generic_transcription_spec(name: str) -> TranscriptionProviderSpec:
    ...
    return TranscriptionProviderSpec(
        name=name,
        default_model="whisper-1",
        adapter="xianaibot.providers.transcription:GenericOpenAITranscriptionProvider",
    )

...
所属模块与项目作用
===================
本文件位于 xianaibot/config 目录，定义 xianaibot 全部配置的数据模型（schema）。
...
from xianaibot.config_base import Base  # 项目内 Pydantic 模型基类
from xianaibot.cron.types import CronSchedule  # 定时任务调度类型
...
if TYPE_CHECKING:
    ...
    from xianaibot.agent.tools.system_io import SystemIoToolConfig
    from xianaibot.agent.tools.web import WebToolsConfig
...
class TranscriptionConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # 由 xianaibot.audio.transcription_registry 校验
    model: str | None = None
...
class TtsConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # 由 xianaibot.audio.tts_registry 校验，None 默认 edge-tts
    model: str | None = None
    voice: str | None = None
...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.xianaibot/workspace"
    ...
    timezone: str = "Asia/Shanghai"  # IANA 时区，如 "Asia/Shanghai"、"America/New_York"
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(default_factory=lambda: _lazy_default("xianaibot.agent.tools.web", "WebToolsConfig"))
    exec: ExecToolConfig = Field(default_factory=lambda: _lazy_default("xianaibot.agent.tools.shell", "ExecToolConfig"))
    file: FileToolsConfig = Field(default_factory=lambda: _lazy_default("xianaibot.agent.tools.filesystem", "FileToolsConfig"))
    cli_apps: CliAppsToolConfig = Field(default_factory=lambda: _lazy_default("xianaibot.agent.tools.cli_apps", "CliAppsToolConfig"))
    my: MyToolConfig = Field(default_factory=lambda: _lazy_default("xianaibot.agent.tools.self", "MyToolConfig"))
    image_generation: ImageGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default("xianaibot.agent.tools.image_generation", "ImageGenerationToolConfig"),
    )
    screenshot: ScreenshotToolConfig = Field(
        default_factory=lambda: _lazy_default("xianaibot.agent.tools.screenshot", "ScreenshotToolConfig"),
    )
    ...
    def _normalize_guard_level(cls, value: Any) -> Any:
        ...
        from xianaibot.security.guard_level import normalize_guard_level
...
class Config(BaseSettings):
    ...
    def _match_provider(
        ...
        *,
        ...
        from xianaibot.providers.registry import (
            PROVIDERS,
            find_by_name,
        )
    ...
    model_config = ConfigDict(env_prefix="XIANAIBOT_", env_nested_delimiter="__")
...
def _resolve_tool_config_refs() -> None:
    ...
    ``from xianaibot.config.schema import ExecToolConfig`` 继续可用。
    """
    import sys
    ...
    from xianaibot.agent.tools.shell import ExecToolConfig
    from xianaibot.agent.tools.system_io import SystemIoToolConfig
    from xianaibot.agent.tools.web import WebFetchConfig, WebSearchConfig, WebToolsConfig

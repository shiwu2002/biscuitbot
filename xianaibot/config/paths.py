...
所属模块与项目作用
===================
本文件位于 xianaibot/config 目录，提供基于当前配置上下文派生的运行时路径。
...
from xianaibot.utils.helpers import ensure_dir  # 确保目录存在（不存在则创建）


def get_config_path() -> Path:
    """获取配置文件路径（惰性导入以打破循环依赖）。

    在调用时委托给 ``xianaibot.config.loader.get_config_path``，从而避免
    在启动阶段导入本模块时触发循环导入。
    """
    from xianaibot.config.loader import get_config_path as _loader_get_config_path
...
def get_workspace_path(workspace: str | Path | None = None) -> Path:
    ...
    未传入 workspace 时回退到默认工作区 ~/.xianaibot/workspace。
    """
    path = Path(workspace).expanduser() if workspace else Path.home() / ".xianaibot" / "workspace"
    return ensure_dir(path)


def is_default_workspace(workspace: str | Path | None) -> bool:
    """判断给定工作区是否解析为 xianaibot 默认工作区路径。"""
    current = Path(workspace).expanduser() if workspace is not None else Path.home() / ".xianaibot" / "workspace"
    default = Path.home() / ".xianaibot" / "workspace"
    return current.resolve(strict=False) == default.resolve(strict=False)


def get_cli_history_path() -> Path:
    """返回共享的 CLI 历史记录文件路径。"""
    return Path.home() / ".xianaibot" / "history" / "cli_history"


def get_bridge_install_dir() -> Path:
    """返回共享的 WhatsApp bridge 安装目录。"""
    return Path.home() / ".xianaibot" / "bridge"


def get_legacy_sessions_dir() -> Path:
    """返回用于迁移回退的遗留全局会话目录。"""
    return Path.home() / ".xianaibot" / "sessions"

...
所属模块与项目作用
===================
本文件位于 xianaibot/config 目录，提供 xianaibot 配置文件的加载、保存与迁移工具。
在项目架构中起到的作用：负责从 JSON 配置文件（默认 ~/.xianaibot/config.json）读取
...
from xianaibot.config.schema import Config, _resolve_tool_config_refs  # 配置数据模型与前置引用解析
from xianaibot.utils.helpers import atomic_write_text  # 配置文件原子写（tmp+rename+fsync）
...
def get_config_path() -> Path:
    ...
    优先级：已设置的路径 > ~/.xianaibot/config.json；
    仅当新路径不存在而遗留的 ~/.biscuitbot/config.json 存在时回退到旧路径。
    """
    if _current_config_path:
        return _current_config_path
    new_path = Path.home() / ".xianaibot" / "config.json"
    if not new_path.exists():
        legacy_path = Path.home() / ".biscuitbot" / "config.json"
...
def _migrate_legacy_home_dir() -> None:
    """首次启动时把遗留的 ``~/.biscuitbot`` 目录整体迁移到 ``~/.xianaibot``。

    旧目录里有 config.json、workspace、history、bridge、sessions 等全部实例数据，
    整体改名即可一次性完成升级。新目录已存在或旧目录不存在时不做任何操作；
    改名失败（如目录被占用）时仅告警，``get_config_path`` 仍可回退读取旧配置。
    """
    new_home = Path.home() / ".xianaibot"
    old_home = Path.home() / ".biscuitbot"
    if new_home.exists() or not old_home.is_dir():
        return
    try:
        old_home.rename(new_home)
        from loguru import logger

        logger.info("Auto-migrated legacy home {} -> {}", old_home, new_home)
    except OSError:
        from loguru import logger

        logger.warning(
            "Found legacy config dir {} but could not move it to {}; "
            "please move it manually, or XiaNaiBot will keep reading the legacy config.",
            old_home,
            new_home,
            exc_info=True,
        )
...
def _migrate_yaml_to_json() -> None:
    ...
    home_xianaibot = Path.home() / ".xianaibot"
    json_path = home_xianaibot / "config.json"
    # 已有 JSON 配置则无需迁移
    if json_path.exists():
        return
    # 依次尝试 config.yaml 与 config.yml
    for yaml_name in ("config.yaml", "config.yml"):
        yaml_path = home_xianaibot / yaml_name
        if yaml_path.exists():
            try:
...
def _apply_ssrf_whitelist(config: Config) -> None:
    ...
    from xianaibot.security.network import configure_ssrf_whitelist
...
def _migrate_config(data: dict) -> dict:
    ...
    # ~/.xianaibot/config.json 文件能继续加载，而非因 pydantic 的 extra-forbidden
    # 校验而失败。
    if "platform" in data:

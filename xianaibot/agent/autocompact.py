...
所属模块与项目作用
===================
本文件位于 ``xianaibot/agent`` 目录，是 Agent 模块中负责会话自动压缩的组件。
在项目架构中起到的作用：
...
from xianaibot.session.manager import Session, SessionManager  # 会话管理器与 Session 对象

if TYPE_CHECKING:
    from xianaibot.agent.memory import Consolidator  # 仅类型检查时导入，避免运行时循环依赖

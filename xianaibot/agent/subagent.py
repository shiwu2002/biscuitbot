...
所属模块与项目作用
===================
本文件位于 ``xianaibot/agent`` 目录，是 Agent 模块中负责“后台子 Agent 执行”的组件。
在项目架构中起到的作用：
...
from xianaibot.agent.hook import AgentHook, AgentHookContext  # 生命周期钩子基类与上下文
from xianaibot.agent.runner import AgentRunner, AgentRunSpec  # Agent 执行器与执行规格
from xianaibot.agent.tools.context import ToolContext  # 工具上下文
from xianaibot.agent.tools.file_state import FileStates  # 文件状态存储
from xianaibot.agent.tools.loader import ToolLoader  # 工具加载器
from xianaibot.agent.tools.registry import ToolRegistry  # 工具注册表
from xianaibot.bus.events import InboundMessage  # 入站消息类型
from xianaibot.bus.queue import MessageBus  # 消息总线
from xianaibot.config.schema import AgentDefaults, ToolsConfig  # Agent 默认值与工具配置
from xianaibot.providers.base import LLMProvider  # LLM 提供商基类
from xianaibot.security.workspace_access import (  # 工作区访问安全控制
    WorkspaceScope,
...
class _SubagentHook(AgentHook):
    ...
    def wants_streaming(self) -> bool:
        ...
        启用后 ``AgentRunner._call_llm`` 对外层墙钟超时（XIANAIBOT_LLM_TIMEOUT_S，
        默认 300s）豁免，改由 provider 层的流式 idle 超时（XIANAIBOT_STREAM_IDLE_TIMEOUT_S）

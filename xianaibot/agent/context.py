...
所属模块与项目作用
===================
本文件位于 ``xianaibot/agent`` 目录，是 Agent 模块中负责上下文装配的核心组件。
在项目架构中起到的作用：
...
from xianaibot.agent.employees import (  # 数字人员工目录存储，按会话注入 persona
    EmployeeStore,
    is_image_avatar,
)
...
# 需求补充与优化指令段：所有会话（主智能体 + 绑定数字员工）统一注入，
# 让「短提示词 → 员工反问」反转为「先补充并优化需求 → 再执行任务」。
# 不做「需求精化/精华」：不把用户需求浓缩、改写或重新提炼成别的表述，
# 而是在保留原始需求原意与明确要求的前提下做补充与优化。
_REQUIREMENT_REFINEMENT_SECTION = (
    "# 需求补充与优化（保留原意，补充并优化，不精化改写）\n\n"
    "收到用户请求后，不要拿原始文字直接动手，也**不要**把用户的需求浓缩、改写或"
    "重新提炼成一套新的说法。先做「需求补充」与「需求优化」，再据此执行：\n"
    "1、需求补充——尊重用户原始需求的表达与原意；用户需求过简或含糊时，"
    "用你的专业经验补齐缺失的背景、关键参数与合理默认值，"
    "不要因为用户没说就搁置，也不要反复向用户追问；\n"
    "2、需求优化——在不动用户原意、不删减用户明确要求的前提下，把需求表达得"
    "更清晰、更可执行（结构清楚、用词准确、粒度合适）；\n"
    "3、确定执行路径——列出执行步骤、会用到的技能或工具"
    "（必要时先用 read_file 读取技能文档），以及输出形式。\n\n"
    "呈现方式：对模糊或复杂的任务，先在回复中以「需求补充与优化」小节简要展示"
    "补充优化后的需求再开始执行，让用户看到自己的需求如何被补全与打磨；"
    "对已经明确的小任务直接执行即可，不必展示，但执行仍应按上述思路展开。"
)
...
class ContextBuilder:
    ...
    def build_system_prompt(
        ...
        # 需求补充与优化：所有会话注入「先补充并优化需求 → 再执行」，把短提示词补全打磨为可执行需求
    ...
    def _employee_capability_runtime_lines(self, session_metadata: Mapping[str, Any] | None) -> list[str]:
        ...
        lines: list[str] = []
        try:
            from xianaibot.apps.cli import CliAppManager

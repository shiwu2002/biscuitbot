...
所属模块与项目作用
===================
本文件位于 ``xianaibot/agent`` 目录，是 Agent 模块中负责记忆持久化与会话合并的核心组件。
在项目架构中起到的作用：
...
if TYPE_CHECKING:
    from xianaibot.providers.base import LLMProvider  # 仅类型检查时导入，避免循环依赖
    from xianaibot.session.manager import SessionManager  # 仅类型检查时导入

...
所属模块与项目作用
===================
本文件位于 xianaibot/agent/tools 目录，是工具系统中负责"长任务目标管理"
...
- 每一轮都会把活跃目标镜像到 Runtime Context 块
  （见 ``xianaibot.session.goal_state.goal_state_runtime_lines``），因此
  压缩无法隐藏它。
...
from xianaibot.agent.tools.context import ContextAware, RequestContext  # 上下文感知混入与请求上下文
from xianaibot.agent.tools.schema import BooleanSchema, StringSchema, tool_parameters_schema  # schema 构造器
from xianaibot.bus.runtime_events import GoalStateChanged, RuntimeEventBus, RuntimeEventContext  # 运行时事件总线与目标状态变更事件
from xianaibot.session.goal_state import (  # 会话目标状态存取工具
...
from xianaibot.utils.helpers import truncate_text  # 文本截断，用于任务清单文案

if TYPE_CHECKING:  # 仅类型检查时导入，避免运行时循环依赖
    from xianaibot.session.manager import SessionManager  # 会话管理器

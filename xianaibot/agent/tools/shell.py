...
所属模块与项目作用
===================
本文件位于 xianaibot/agent/tools 目录，是工具系统中的 shell 执行组件。
...
from xianaibot.agent.tools.context import current_request_session_key  # 当前请求会话键
from xianaibot.agent.tools.exec_session import (  # 执行会话管理
...
from xianaibot.agent.tools.sandbox import wrap_command  # 沙箱命令包装
from xianaibot.agent.tools.schema import (  # JSON Schema 类型
    BooleanSchema,
    IntegerSchema,
    StringSchema,
    tool_parameters_schema,
)
from xianaibot.config.paths import get_media_dir  # 媒体目录路径
from xianaibot.config_base import Base  # 配置基类
from xianaibot.security.guard_level import GuardPolicy  # 守卫策略
...
class ExecTool(Tool):
    ...
    def _wrap_path_export(
        ...
        segments = []
        if venv_bin:
            env["XIANAIBOT_VENV_BIN"] = venv_bin
            segments.append("$XIANAIBOT_VENV_BIN")
        if self.path_prepend:
            env["XIANAIBOT_PATH_PREPEND"] = self.path_prepend
            segments.append("$XIANAIBOT_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["XIANAIBOT_PATH_APPEND"] = self.path_append
            segments.append("$XIANAIBOT_PATH_APPEND")
    ...
    def _guard_command(
        ...
        *,
        ...
        from xianaibot.security.network import contains_internal_url

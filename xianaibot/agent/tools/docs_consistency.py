...
所属模块与项目作用
===================
本文件位于 xianaibot/agent/tools 目录，是工具系统的文档一致性检查组件。
...
if TYPE_CHECKING:
    from xianaibot.agent.tools.base import Tool  # 工具基类，仅用于类型提示
    from xianaibot.agent.tools.registry import ToolRegistry  # 工具注册表，仅用于类型提示
...
def _resolve_source_file(tool: "Tool") -> str:
    ...
    if module and module.startswith("xianaibot."):
        parts = module.split(".")

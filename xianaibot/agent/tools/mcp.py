"""MCP 客户端：连接 MCP 服务器并将其工具包装为原生 xianaibot 工具。

所属模块与项目作用
===================
本文件位于 xianaibot/agent/tools 目录，是工具系统与 Model Context
Protocol (MCP) 生态的集成层。它负责：
- 连接配置的 MCP 服务器（支持 stdio / SSE / streamableHttp 三种传输）。
- 将 MCP 服务器暴露的 tools / resources / prompts 包装为 xianaibot 原生
  ``Tool`` 子类，注册到工具注册表。
...
class MCPToolWrapper(_MCPWrapperBase):
    """将单个 MCP 服务器工具包装为 xianaibot Tool。
...
class MCPResourceWrapper(_MCPWrapperBase):
    """将 MCP 资源 URI 包装为只读的 xianaibot Tool。
...
class MCPPromptWrapper(_MCPWrapperBase):
    """将 MCP prompt 包装为只读的 xianaibot Tool。
    ...
    for item in structured[:8]:
        ...
        if connected_server_names is not None and raw_name not in connected_server_names:
            lines.append(
                ...
                "run the preset test, and restart 夏奈儿 only if hot reload is unavailable."
            )
            continue
        # 情况 3：正常可用
        lines.append(
...
async def reload_servers(state: Any, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        try:
            from xianaibot.config.loader import load_config, resolve_config_env_vars
            ...
            return {
                "ok": False,
                "message": "Could not reload MCP config. Restart 夏奈儿 to pick up changes.",
        ...
        if failed:
            ...
            message = "MCP connections refreshed without restarting 夏奈儿."
        else:
            message = "MCP config reloaded without restarting 夏奈儿."
...
async def request_mcp_reload(bus: Any, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "message": "MCP hot reload timed out. Restart 夏奈儿 to pick up changes.",
            "requires_restart": True,
        }
...
async def handle_runtime_control(state: Any, msg: InboundMessage, registry: ToolRegistry) -> bool:
    ...
    try:
        ...
        result = {
            "ok": False,
            "message": "MCP hot reload failed. Restart 夏奈儿 to pick up changes.",
            "requires_restart": True,
            "error": str(exc),
        }
    # 将结果通过 ack Future 回传

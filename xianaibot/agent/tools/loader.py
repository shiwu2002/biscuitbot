...
所属模块与项目作用
==================
本文件位于 xianaibot/agent/tools 目录，是工具系统的加载与发现组件。
在项目架构中起到的作用：扫描 ``xianaibot.agent.tools`` 包下的所有模块，
...
class ToolLoader:
    ...
    def __init__(self, package: Any = None, *, test_classes: list[type[Tool]] | None = None):
        if package is None:
            # 默认扫描 xianaibot.agent.tools 包
            import xianaibot.agent.tools as _pkg
            package = _pkg
    ...
    def _discover_plugins(self) -> dict[str, type[Tool]]:
        ...
        try:
            # 读取 xianaibot.tools 入口点组
            eps = entry_points(group="xianaibot.tools")
        except Exception:
            return plugins
        for ep in eps:
            try:

...
所属模块与项目作用
==================
本文件位于 xianaibot/agent/tools 目录，是工具系统的主动消息投递组件。
...
from xianaibot.agent.tools.path_utils import resolve_workspace_path  # 工作区路径解析
from xianaibot.agent.tools.schema import ArraySchema, StringSchema, tool_parameters_schema  # schema 构造器
...
from xianaibot.config.paths import get_workspace_path  # 工作区路径获取函数
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        返回:
            发送结果字符串；出错时返回错误信息。
        """
        from xianaibot.utils.helpers import strip_think

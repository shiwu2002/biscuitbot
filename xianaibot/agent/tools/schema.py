"""JSON Schema 片段类型：所有类型都继承自 :class:`~xianaibot.agent.tools.base.Schema`，
用于描述工具参数的文档与约束。

所属模块与项目作用
===================
本文件位于 xianaibot/agent/tools 目录，是工具系统的 schema 定义层组件。
它为工具参数提供类型化的 schema 构造器（字符串、整数、数字、布尔、数组、
对象），最终通过 ``to_json_schema()`` 输出符合 JSON Schema 规范的字典，
供 :class:`~xianaibot.agent.tools.base.Tool` 发送给模型并校验入参。

主要 API：
- ``to_json_schema()``：返回与
  :meth:`~xianaibot.agent.tools.base.Schema.validate_json_schema_value` /
  :class:`~xianaibot.agent.tools.base.Tool` 兼容的字典。
- ``validate_value(value, path)``：根据本 schema 校验单个值，返回错误信息
  列表（空列表表示有效）。

共享的校验与片段规范化逻辑位于 :class:`~xianaibot.agent.tools.base.Schema`
的类方法上。
...
from xianaibot.agent.tools.base import Schema  # Schema 基类，提供共享校验与片段规范化

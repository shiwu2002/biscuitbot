...
所属模块与项目作用
===================
本文件位于 xianaibot/agent/tools 目录，是工具系统渐进式发现架构的核心
...
if TYPE_CHECKING:  # 仅类型检查时导入，避免循环依赖
    from xianaibot.agent.tools.usage_stats import UsageStats  # 使用统计，用于冷存储轮换
    ...
    def validate_tool_file(file_path: str, docs_md_path: str) -> tuple[Tool | None, str | None]:
        ...
        if not file_path or not os.path.isfile(file_path):
            return None, (
                ...
                f"工具必须继承 xianaibot.agent.tools.base.Tool 并实现 name/description/parameters/execute。"
            )
        ...
        try:
            ...
            return None, (
                f"工具文件导入失败：{e}。"
                f"请检查 Python 语法、导入语句（需要 from xianaibot.agent.tools.base import Tool）。"
            )
        ...
        if not tool_classes:
            return None, (
                "未找到 Tool 子类。工具类必须继承 xianaibot.agent.tools.base.Tool，"

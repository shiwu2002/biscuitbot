...
所属模块与项目作用
===================
本文件位于 xianaibot/agent/tools 目录，是工具系统中的数字员工调用组件。
...
``.xianaibot/tool-results/employees``，内联文本保留精华并附完整成果的
...
from xianaibot.agent.tools.schema import StringSchema, tool_parameters_schema  # JSON Schema 类型
from xianaibot.security.workspace_access import current_workspace_scope  # 当前工作区作用域
from xianaibot.utils.helpers import atomic_write_text, ensure_dir, safe_filename  # 原子写入与目录/文件名工具
...
def _persist_employee_result(workspace: Path, employee_id: str, content: str) -> Path:
    ...
    """
    root = ensure_dir(workspace / ".xianaibot" / "tool-results" / "employees")

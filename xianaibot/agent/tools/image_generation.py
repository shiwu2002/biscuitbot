...
所属模块与项目作用
==================
本文件位于 xianaibot/agent/tools 目录，是工具系统的图像生成组件。
...
from xianaibot.config.paths import get_media_dir  # 媒体目录获取函数
from xianaibot.config_base import Base  # 配置基类
from xianaibot.providers.image_generation import (  # 图像生成提供商抽象
...
from xianaibot.security.workspace_access import current_tool_workspace  # 当前工具工作区访问
from xianaibot.security.workspace_policy import WorkspaceBoundaryError, resolve_allowed_path  # 工作区路径安全策略
from xianaibot.utils.artifacts import (  # artifact 存储工具
    ArtifactError,
    generated_image_tool_result,
    store_generated_image_artifact,
)
from xianaibot.utils.helpers import detect_image_mime  # 图片 MIME 类型探测

if TYPE_CHECKING:  # 仅类型检查时导入，避免运行时循环依赖
    from xianaibot.config.schema import ProviderConfig
...
class ImageGenerationTool(Tool):
    ...
    def _resolve_reference_image(self, value: str) -> str:
        ...
        try:
            ...
            # 参考图片必须在工作区或 xianaibot 媒体目录内
            raise ImageGenerationError(
                "参考图路径越界：请把参考图放到工作区或 xianaibot 媒体目录内"
            ) from exc

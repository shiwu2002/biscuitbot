...
所属模块与项目作用
===================
本文件位于 xianaibot/agent/tools 目录，是数字员工调用链路的检索组件。
...
（``xianaibot.webui.talent_market`` 拉取的注册表目录）。
...
from xianaibot.agent.tools.schema import IntegerSchema, StringSchema, tool_parameters_schema  # JSON Schema 类型
...
class DiscoverEmployeesTool(Tool):
    ...
    async def execute(self, query: str, limit: int | None = None, **kwargs: Any) -> str:
        ...
        note_parts: list[str] = []
        try:
            from xianaibot.webui import talent_market as tm  # 懒加载，避免工具包与 webui 隐式耦合

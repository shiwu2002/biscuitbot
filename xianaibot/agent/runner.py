...
所属模块与项目作用
===================
本文件位于 ``xianaibot/agent`` 目录，是 Agent 模块中负责“工具型 LLM 执行循环”
...
from xianaibot.agent.hook import (  # 生命周期钩子基类与上下文
    AgentHook,
    AgentHookContext,
    AgentRunHookContext,
)
from xianaibot.agent.tools.registry import ToolRegistry  # 工具注册表
from xianaibot.providers.base import (  # LLM 提供商基类、响应与工具调用请求类型
    LLMProvider,
    LLMResponse,
    ToolCallRequest,
)
from xianaibot.utils.file_edit_events import (  # 文件编辑流式事件追踪与构建
...
from xianaibot.utils.file_edit_events import (  # 单文件追踪器（兼容旧测试/扩展的 monkeypatch 入口）
...
from xianaibot.utils.progress_events import (  # 进度事件辅助
    invoke_file_edit_progress,  # 触发文件编辑进度事件
    on_progress_accepts_file_edit_events,  # 判断进度回调是否接受文件编辑事件
)
from xianaibot.utils.prompt_templates import render_template  # 模板渲染（如 max_iterations 提示）
from xianaibot.utils.runtime import (  # 运行时辅助消息与判定
...
class AgentRunner:
    ...
    async def _request_model(
        ...
        if timeout_s is None:
            ...
            # Set XIANAIBOT_LLM_TIMEOUT_S=0 to disable.
            raw = os.environ.get("XIANAIBOT_LLM_TIMEOUT_S", "300").strip()
            try:
                timeout_s = float(raw)
        ...
        # (XIANAIBOT_STREAM_IDLE_TIMEOUT_S). Do not also apply the outer wall-clock
        # LLM timeout here, or healthy long reasoning streams can be killed just
        # because total elapsed time exceeded XIANAIBOT_LLM_TIMEOUT_S.

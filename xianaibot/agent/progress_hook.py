...
所属模块与项目作用
===================
本文件位于 ``xianaibot/agent`` 目录，是 Agent 模块中负责“进度/流式 UI 适配”的组件。
在项目架构中起到的作用：
...
from xianaibot.agent.hook import AgentHook, AgentHookContext  # 钩子基类与迭代上下文
from xianaibot.utils.helpers import IncrementalThinkExtractor, strip_think  # think 增量提取与剥离
from xianaibot.utils.progress_events import (  # 进度事件构建与调用辅助
    build_tool_event_finish_payloads,
    build_tool_event_start_payload,
    invoke_on_progress,
    on_progress_accepts_tool_events,
)
from xianaibot.utils.tool_hints import format_tool_hints  # 工具提示格式化

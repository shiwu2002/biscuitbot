...
所属模块与项目作用
===================
本文件位于 ``xianaibot/agent`` 目录，是 Agent 模块的核心处理引擎。
在项目架构中起到的作用：
...
from xianaibot.agent import context as agent_context  # Agent 上下文相关辅助函数集合
from xianaibot.agent import model_presets as preset_helpers  # 模型预设辅助函数
from xianaibot.agent.autocompact import AutoCompact  # 空闲会话自动压缩器
from xianaibot.agent.context import ContextBuilder  # 上下文构建器
from xianaibot.agent.cron_turns import CronTurnCoordinator  # cron 轮次协调器
from xianaibot.agent.hook import AgentHook, CompositeHook  # 生命周期钩子与组合钩子
from xianaibot.agent.memory import Consolidator  # 记忆合并器
from xianaibot.agent.progress_hook import AgentProgressHook  # 进度/流式输出钩子
from xianaibot.agent.runner import (  # Runner 与运行规格
    _MAX_INJECTIONS_PER_TURN,
    AgentRunner,
    AgentRunSpec,
)
from xianaibot.agent.subagent import SubagentManager  # 子代理管理器
from xianaibot.agent.tools.context import (  # 请求上下文绑定
    RequestContext,
    bind_request_context,
    reset_request_context,
)
from xianaibot.agent.tools.file_state import (  # 文件状态跟踪
    FileStateStore,
    bind_file_states,
    reset_file_states,
)
from xianaibot.agent.tools.message import MessageTool  # 消息工具（用于抑制响应等判断）
from xianaibot.agent.tools.registry import ToolRegistry  # 工具注册表
from xianaibot.agent.tools.self import MyTool  # 自我状态查询工具
from xianaibot.bus.events import InboundMessage, OutboundMessage  # 入/出站消息类型
from xianaibot.bus.progress import build_bus_progress_callback  # 总线进度回调构建
from xianaibot.bus.queue import MessageBus  # 消息总线
from xianaibot.bus.runtime_events import (  # 运行时事件总线与发布器
    RuntimeEventBus,
    RuntimeEventPublisher,
    ensure_runtime_event_publisher,
)
from xianaibot.command import (  # 命令路由与内置命令注册
    CommandContext,
    CommandRouter,
    register_builtin_commands,
)
from xianaibot.config.schema import (  # Agent 默认配置与模型预设配置
    AgentDefaults,
    ModelPresetConfig,
)
from xianaibot.cron.session_turns import (  # cron 会话轮次辅助
    cron_history_overrides,
)
from xianaibot.providers.base import LLMProvider  # LLM 提供商基类
from xianaibot.providers.factory import ProviderSnapshot  # 提供商快照
from xianaibot.security.workspace_access import (  # 工作区访问范围解析与绑定
    WorkspaceScopeResolver,
    bind_workspace_scope,
    reset_workspace_scope,
)
from xianaibot.session import turn_continuation  # 轮次延续相关逻辑
from xianaibot.session.goal_state import (  # 目标状态相关
    goal_state_runtime_lines,
    runner_wall_llm_timeout_s,
    sustained_goal_active,
)
from xianaibot.session.keys import UNIFIED_SESSION_KEY, session_key_for_channel  # 会话 key 生成
from xianaibot.session.manager import Session, SessionManager  # 会话与 会话管理器
from xianaibot.utils.document import (  # 文档抽取与附件引用
    extract_documents,
    reference_non_image_attachments,
)
from xianaibot.utils.helpers import image_placeholder_text  # 图片占位文本
from xianaibot.utils.helpers import truncate_text as truncate_text_fn  # 文本截断（别名）
from xianaibot.utils.image_generation_intent import image_generation_prompt  # 图像生成意图识别
from xianaibot.utils.llm_runtime import LLMRuntime  # LLM 运行时信息
from xianaibot.utils.runtime import (  # 运行时常量
    EMPTY_FINAL_RESPONSE_MESSAGE,
)

if TYPE_CHECKING:
    from xianaibot.config.schema import (
        ChannelsConfig,
        ProviderConfig,
        ToolsConfig,
    )
...
class AgentLoop:
    ...
    def __init__(
        ...
        # XIANAIBOT_MAX_CONCURRENT_REQUESTS：<=0 表示不限；默认 3。
        _max = int(os.environ.get("XIANAIBOT_MAX_CONCURRENT_REQUESTS", "3"))
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from xianaibot.providers.factory import make_provider
        ...
        if vision_provider_loader is None:
            from xianaibot.providers.factory import build_vision_provider
    ...
    def _register_default_tools(self) -> None:
        ...
        if self.tools_config.screenshot.enable:
            from xianaibot.agent.tools.screenshot import ScreenshotTool
        ...
        from xianaibot.agent.tools.register_tool import (
            RegisterToolTool,
            UnregisterToolTool,
        )
    ...
    def _set_tool_context(
        ...
        from xianaibot.agent.tools.context import ContextAware
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        try:
            result = await self.runner.run(AgentRunSpec(
                ...
                # Sustained goals may legitimately exceed XIANAIBOT_LLM_TIMEOUT_S; idle stall
                # is still capped by XIANAIBOT_STREAM_IDLE_TIMEOUT_S in streaming providers.

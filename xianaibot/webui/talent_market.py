...
所属模块与项目作用
===================
本文件位于 ``xianaibot/webui`` 目录，负责数字员工人才市场的目录拉取与安装：

- 用户提供一个 http/https 注册表 URL，该 URL 托管一份员工目录 JSON
  （形如 ``{"schema":"talent-market.v1","meta":{"updated":...},"employees":[...]}``）；
- ``_fetch_talent_catalog`` 仿 ``xianaibot/apps/cli/service.py`` 的 ``_fetch_registry``
...
from xianaibot.agent.employees import EmployeeStore, EmployeeValidationError, _slugify
...
def read_talent_market_registry_urls(config_path: Path | None = None) -> list[str]:
    ...
    CLI ``xianaibot talent-market add/set <url>`` 修改。这里直接读原始 JSON，

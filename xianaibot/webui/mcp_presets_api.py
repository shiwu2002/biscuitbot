...
from xianaibot.config.loader import load_config, resolve_config_env_vars, update_config
from xianaibot.config.paths import get_runtime_subdir
from xianaibot.config.schema import MCPServerConfig
from xianaibot.security.network import validate_url_target
...
def _custom_manifest(name: str, cfg: MCPServerConfig) -> dict[str, Any]:
    ...
    return capability_manifest(
        capability_id=name,
        display_name=name,
        description="Custom MCP server from 夏奈儿 config.",
        category="custom",
        source="mcp-custom",
        brand_color="#64748B",
        runtime="mcp",
...
async def mcp_presets_test_action(query: QueryParams) -> dict[str, Any]:
    ...
    from xianaibot.agent.tools.mcp import connect_mcp_servers

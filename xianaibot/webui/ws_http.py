...
from xianaibot.command.builtin import builtin_command_palette
from xianaibot.cron.session_turns import is_bound_cron_job
from xianaibot.cron.types import CronJob, CronSchedule
from xianaibot.utils.subagent_channel_display import scrub_subagent_messages_for_channel
from xianaibot.webui.capabilities_api import (
...
from xianaibot.webui.media_gateway import WebUIMediaGateway
from xianaibot.webui.session_automations import (
    all_automations_payload,
    serialize_automation_jobs,
    session_automation_jobs,
    session_automations_payload,
)
from xianaibot.webui.session_list_index import list_webui_sessions
from xianaibot.webui.sidebar_state import (
...
from xianaibot.webui.thread_disk import delete_webui_thread
from xianaibot.webui.transcript import build_webui_thread_response
from xianaibot.webui.workspaces import WebUIWorkspaceController
...
def _resolve_avatar_path(name: str) -> Path | None:
    """定位员工头像文件，按优先级：源码 ``images/bot/`` → 打包内置 ``xianaibot/avatars/``
    ...
    if not _AVATAR_NAME_RE.match(name or ""):
        return None
    here = Path(__file__).resolve().parent  # xianaibot/webui
    candidates = (
        here.parent.parent / "images" / "bot" / name,  # 源码运行（仓库根 images/bot）
        here.parent / "avatars" / name,  # 打包（--add-data 内置到 xianaibot/avatars）
...
def _get_user_avatar_dir() -> Path:
    ...
    from xianaibot.config.paths import get_runtime_subdir
...
class GatewayHTTPHandler:
    ...
    def __init__(
        ...
        *,
        ...
        from xianaibot.webui.weixin_login import WeixinLoginManager

        self._weixin_login = WeixinLoginManager()

        from xianaibot.webui.settings_api import runtime_capabilities as _rc
        from xianaibot.webui.settings_routes import WebUISettingsRouter
    ...
    def _handle_bootstrap(self, connection: Any, request: Any) -> Response:
        ...
        # needs_setup 判定基于完整 xianaibot 配置（self.config 只是 websocket 频道配置）
    ...
    def _sessions_list_payload(self) -> dict[str, Any]:
        ...
        from xianaibot.session.webui_turns import websocket_turn_wall_started_at
    ...
    def _handle_desktop_restart(self, request: WsRequest) -> Response:
        ...
        桌面壳没有注入 ``window.xianaibotHost``，前端无法直接调用宿主的
    ...
    def _handle_desktop_open_logs(self, request: WsRequest) -> Response:
        ...
        桌面壳未注入 ``window.xianaibotHost``，前端无法直接调用宿主的
        ...
        from xianaibot.config.paths import get_logs_dir
    ...
    def _handle_desktop_export_diagnostics(self, request: WsRequest) -> Response:
        ...
        from xianaibot.config.paths import get_data_dir
        from xianaibot.utils.helpers import ensure_dir

        diagnostics_dir = ensure_dir(get_data_dir() / "diagnostics")
        path = diagnostics_dir / f"xianaibot-diagnostics-{time.strftime('%Y%m%d-%H%M%S')}.txt"
    ...
    def _build_diagnostics_report(self) -> str:
        ...
        from xianaibot.config.loader import load_config
        from xianaibot.config.paths import (
            get_config_path,
            get_data_dir,
            get_logs_dir,
            get_workspace_path,
    ...
    async def _handle_webui_talent_catalog(self, request: WsRequest) -> Response:
        ...
        注册表 URL 只能由后台 CLI 设置（``xianaibot talent-market add/set <url>``），
    ...
    def _handle_webui_setup_complete(self, request: WsRequest) -> Response:
        ...
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        from xianaibot.config.loader import update_config
    ...
    async def _handle_webui_weixin_login_status(self, request: WsRequest) -> Response:
        ...
        ``~/.xianaibot/weixin/account.json``，此处额外把 ``channels.weixin.enabled``

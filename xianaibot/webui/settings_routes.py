...
from xianaibot.agent.tools.mcp import request_mcp_reload
...
from xianaibot.webui.cli_apps_api import cli_apps_action, cli_apps_payload
from xianaibot.webui.http_utils import query_first as _query_first
from xianaibot.webui.mcp_presets_api import mcp_presets_settings_action
...
from xianaibot.webui.version_check import check_for_update

QueryParams = dict[str, list[str]]

_MCP_VALUES_HEADER = "X-Xianaibot-MCP-Values"
...
class WebUISettingsRouter:
    ...
    async def _handle_settings_self_update(self, request: WsRequest) -> Response:
        """Run pip install --upgrade xianaibot and return the result."""
        if not self._authorized(request):
            return self._unauthorized()
        ...
        def _run_pip_upgrade() -> dict[str, Any]:
            try:
                result = subprocess.run(
                    [sys.executable, "-m", "pip", "install", "--upgrade", "xianaibot"],
                    capture_output=True,
                    text=True,
                    timeout=120,
                )
                ...
                if result.returncode == 0:
                    # 解析安装的版本
                    import re
                    match = re.search(r"Successfully installed xianaibot-([^\s]+)", output)

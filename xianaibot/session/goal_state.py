...
所属模块与项目作用
===================
本文件位于 xianaibot/session 目录，提供持久化目标（如 ``long_task`` / ``complete_goal``）
相关的会话元数据读写辅助。
...
from xianaibot.session.manager import SessionManager  # 会话管理器，用于按需读取会话元数据
from xianaibot.utils.helpers import truncate_text  # 文本截断，用于任务清单渲染与 WebSocket 推送
...
def runner_wall_llm_timeout_s(
    ...
    *,
    ...
    """:class:`~xianaibot.agent.runner.AgentRunner` 流式调用 LLM 时的墙钟时限。

    当为持久化目标 turn 时返回 ``0.0`` 表示禁用请求外的 ``asyncio.wait_for``；
    返回 ``None`` 表示使用 ``XIANAIBOT_LLM_TIMEOUT_S``。当调用方已持有本 turn 的
    :attr:`~xianaibot.session.manager.Session.metadata` 时可直接传入内存中的 ``metadata``。
    """

...
所属模块与项目作用
===================
本文件位于 xianaibot/session 目录（虽名称含 webui，但属于 session 模块）。
...
from xianaibot.bus import progress as bus_progress  # 通用进度回调构造
from xianaibot.bus.events import InboundMessage, OutboundMessage  # 消息数据类
from xianaibot.bus.queue import MessageBus  # 异步消息总线
from xianaibot.bus.runtime_events import (  # 运行时事件类型与总线
    AgentTraceEvent,
    GoalStateChanged,
    RuntimeEventBus,
    RuntimeEventContext,
...
from xianaibot.cron.session_turns import CRON_HISTORY_META  # cron 历史消息元数据键
from xianaibot.providers.base import LLMProvider  # LLM 提供商基类
from xianaibot.session.goal_state import goal_state_ws_blob  # 目标状态 WebSocket 快照
from xianaibot.session.manager import Session, SessionManager  # 会话与管理器
from xianaibot.utils.helpers import strip_think, truncate_text  # 文本处理辅助
from xianaibot.utils.llm_runtime import LLMRuntime  # LLM 运行时封装

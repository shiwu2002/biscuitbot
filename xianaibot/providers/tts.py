...
from xianaibot.providers.base import resolve_api_endpoint_url  # 共享的端点 URL 解析
...
class EdgeTtsProvider:
    ...
    async def synthesize(self, text: str, output_path: str | Path) -> str:
        ...
        try:
            ...
            raise TtsError(
                "未安装 edge-tts，请执行 pip install 'xianaibot[tts]'"
            ) from exc
        path = _ensure_output_path(output_path)
        try:
...
class DashScopeTtsProvider:
    ...
    async def synthesize(self, text: str, output_path: str | Path) -> str:
        ...
        try:
            ...
            raise TtsError(
                "未安装 dashscope SDK，请执行 pip install 'xianaibot[tts]'"
            ) from exc

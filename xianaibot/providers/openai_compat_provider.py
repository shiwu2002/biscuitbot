...
本模块是 xianaibot 对接各家 LLM 服务（OpenAI、DeepSeek、阿里通义、智谱、
...
from xianaibot.providers.base import (  # 抽象基类与共享数据结构
    LLMProvider,
    LLMResponse,
    ToolCallRequest,
...
from xianaibot.providers.openai_responses import (  # Responses API 转换与解析工具
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    ...
    from xianaibot.providers.registry import ProviderSpec  # Provider 元数据
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/xianaibot",
    "X-OpenRouter-Title": "XiaNaiBot",
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("XIANAIBOT_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """判断是否需要为请求附加 xianaibot 归因 header（OpenRouter 默认附加）。

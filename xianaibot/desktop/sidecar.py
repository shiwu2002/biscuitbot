...
Runs the xianaibot gateway without any window and announces readiness on
stdout with a single handshake line the Tauri shell parses::

    XIANAIBOT_GATEWAY_READY <host> <port>

On startup failure it prints::

    XIANAIBOT_GATEWAY_ERROR <message>
...
def ensure_runtime(config: Any = None) -> Any:
    ...
    """
    from xianaibot.config.loader import get_config_path, load_config, save_config
    from xianaibot.desktop.app import _ensure_websocket_enabled
...
def main(config: Any = None) -> None:
    ...
    from xianaibot.desktop.app import resolve_websocket_endpoint, start_gateway
    ...
    try:
        ...
        from xianaibot.desktop.app import _set_websocket_port
        ...
        if handle.wait_until_ready(timeout=_READY_TIMEOUT_S):
            print(f"XIANAIBOT_GATEWAY_READY {handle.host} {handle.port}", flush=True)
        else:
            if handle.errors:
        ...
        print(f"XIANAIBOT_GATEWAY_ERROR {exc}", flush=True)
...
def _watch_pid() -> int | None:
    ...
    打包场景：Tauri 壳通过环境变量 ``XIANAIBOT_PARENT_PID`` 传入自身 PID，
    ...
    未设置（``xianaibot sidecar`` 直接跑）时返回 None，退化为 getppid 检测。
    """
    raw = os.environ.get("XIANAIBOT_PARENT_PID")

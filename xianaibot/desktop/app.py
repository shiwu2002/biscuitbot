...
Starts the xianaibot gateway in a daemon thread without opening any window;
the Tauri sidecar (:mod:`xianaibot.desktop.sidecar`) reuses this runtime to
...
def resolve_websocket_endpoint(config: Any) -> tuple[str, int]:
    ...
    与 ``xianaibot gateway`` 命令一致：WebUI 走 ``config.channels.websocket``

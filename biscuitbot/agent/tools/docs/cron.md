...
| tz | string | 否 | 工具默认时区 | IANA 时区（如 'Asia/Shanghai', 'America/Vancouver'），仅用于 cron_expr，不能与 at 同时使用 |
...
# 一次性任务（at 使用工具默认时区，不能指定 tz）
cron(action="add", at="2026-03-15T15:00:00", message="提醒用户会议")
...
- action="list" 无需额外参数
- 跨时区场景需明确指定 tz（仅对 cron_expr 有效，at 始终使用工具默认时区）
